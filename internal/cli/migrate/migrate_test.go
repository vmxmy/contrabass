...
func TestLoadMigrateCloudSourceReadsWorkflowTeamStateAndBoard(t *testing.T) {
	...
	source, err := loadMigrateCloudSource(context.Background(), cloudOptions{
		TeamName: "alpha",
		RootDir:  root,
	})
	...
	assert.Equal(t, cloudBoardEntry{
		IssueRef:         "CB-1",
...
func TestUploadMigrateCloudSourceSkipsExistingConfigHashAndBoardExternalID(t *testing.T) {
	...
	var boardRefreshEntries []cloudBoardEntry
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodPost && r.URL.Path == "/v1/teams/alpha/board/refresh":
			var body struct {
				Entries []cloudBoardEntry `json:"entries"`
			}
	...
	result, err := uploadMigrateCloudSource(context.Background(), source, cloudOptions{
...
func TestUploadMigrateCloudSourceSecondRunSkipsExistingBoardExternalIDs(t *testing.T) {
	...
	var boardEntries []cloudBoardEntry
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodGet && r.URL.Path == "/v1/teams/alpha/board":
			_ = json.NewEncoder(w).Encode(map[string]any{
				"board": map[string][]cloudBoardEntry{
					"open":    boardEntries,
					"claimed": {},
					"running": {},
					"done":    {},
	...
	first, err := uploadMigrateCloudSource(context.Background(), source, cloudOptions{
		APIBaseURL: server.URL,
		AuthToken:  "migrate-token",
		HTTPClient: server.Client(),
	})
	...
	second, err := uploadMigrateCloudSource(context.Background(), source, cloudOptions{
		APIBaseURL: server.URL,
		AuthToken:  "migrate-token",
		HTTPClient: server.Client(),
	})
...
func TestLoadMigrateCloudSourceRejectsMissingTeam(t *testing.T) {
	...
	_, err := loadMigrateCloudSource(context.Background(), cloudOptions{RootDir: t.TempDir()})
	require.Error(t, err)
