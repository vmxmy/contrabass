...
// runConfig carries the resolved CLI flags and build metadata into the
// single-host server runtime. Build metadata (version/commit/date) is injected
// by the cmd wiring layer from ldflags variables that live in package main.
...
// runServer is the single-host server entry point wired into the root
// command's RunE in localonly builds.
func runServer(c runConfig) error {
	...
	// 8. Create agent runner (reuses createRunner from the team CLI facade)
	...
	orch.SetBuildInfo(orchestrator.BuildInfo{Version: c.Version, Commit: c.Commit, Date: c.Date})
...
// Duplicated from package main (a trivial pure helper) so the server runtime
...
// Duplicated from package main (a trivial pure helper).
