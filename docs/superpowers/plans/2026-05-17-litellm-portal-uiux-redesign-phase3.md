...
- **Toolchain HARD RULES (run from `cloud/`):**
  ...
  - **GATE REALITY (verified — no CI backstop for this suite):** NO CI workflow runs `cd cloud && bun run test` (`ci.yml` Test = `make test-quick` = go + `packages/dashboard` + landing; `cloud-ci.yml` = build/deploy-dry; `portal-e2e.yml` = Playwright only). The **§A-GATE local vitest sweep is the ONLY gate** these tests get — "CI is green so it's fine" is FALSE and must never be the justification. Everything in the sweep must be GENUINELY green. The ONLY tolerated exclusions are TWO, by-name, isolated-confirmed: (a) the happy-dom `http://localhost:3000/kumo.css` fetch-timeout flake (rotates across `index.test.ts`/`security.test.ts`/`usage-overview-routes.test.ts` — files Phase-3 does not modify; re-confirm environmental via an ISOLATED re-run of the named file); (b) the pre-existing-on-`main` `usage-overview ?window=90d` failure (pre-dates Phase-3; re-confirm still failing identically on `main`). Both are excluded EXPLICITLY BY NAME after isolated re-confirmation, NOT rationalized as "CI covers it". Any other red = a real defect that blocks §B (e.g. the pre-existing `hydration.test.tsx` "tenant_admin full-nav shell at /" #418 — that is NOT tolerated; it is FIXED by Task 0B, since the §A-GATE is its only gate).
...
| `cloud/src/litellm-portal/a11y/use-has-hydrated.ts` (+`.test.ts`) | **Task 0B** (pre-existing #418 fix): SSR-safe hydration gate (`false` on SSR+client-first-render, `true` post-`useEffect`) | Create |
| `cloud/src/litellm-portal/components/ssr-safe-skeleton.tsx` (+`.test.tsx`) | **Task 0B**: deterministic `Math.random`-free SSR placeholder, swaps to Kumo `SkeletonLine` post-hydration — kills the pre-existing shipped Kumo-unseeded-random #418 on the SSR shell path | Create |
| `cloud/src/litellm-portal/tenant-portal/screens/overview.tsx` | **Task 0B**: replace the SSR-path `SkeletonLine` loading branches with `SsrSafeSkeleton` (the exact branches Step-1 pinpoints; Task 2 later migrates the screen to `Panel*`) | Modify |
| `cloud/src/litellm-portal/hydration.test.tsx` | **Task 0B PRIMARY oracle (UNCHANGED — not edited):** the pre-existing "tenant_admin full-nav shell at /" #418 case goes RED→GREEN; 7/7. (Also Task-3's #418 parity host — Task 3 extends it; Task 0B does NOT modify it.) | Not modified by 0B |
...
| `cloud/src/litellm-portal/components/panel-state.tsx` | The four state contracts: `PanelSkeleton` (built on Task-0B's `SsrSafeSkeleton`, NOT raw Kumo `SkeletonLine` — inherits the #418-safety), `PanelEmpty`, `PanelError`, `PanelLoading` (wrap Kumo `Empty`/`Banner`/`Loader`) | Create |
...
| **V2.0 §F fold-in files (6 items):** | | |
| `cloud/src/litellm-portal/a11y/type-scale.test.ts` | §F.5: reads DESIGN.md Typography table, asserts ≤7 tiers + heading:body ≤2× | Create |
| `cloud/src/litellm-portal/components/density.tsx` (+`.test.tsx`) | §F.1: add `resolveDensity` pure SSR-safe resolver + `densityClasses().cell/.row` table tiers (extends the Task-2 module) | Modify |
| `cloud/src/litellm-portal/schemas.ts` | §F.1: add `density: z.enum(["comfortable","compact"]).optional()` to `UserPreferences` (additive) | Modify |
| `cloud/src/litellm-portal/components/density-toggle.tsx` (+`.test.tsx`) | §F.1: controlled comfortable↔compact toggle (Ops chrome only) | Create |
| `cloud/src/litellm-portal/errors/error-messages.ts` (+`.test.ts`) | §F.3: code→human 3-element message map (single source) | Create |
| `cloud/src/litellm-portal/errors/extract-error.ts` | §F.3: shared `extractError` routing through the map (replaces the 2 duplicated locals) | Create |
| `cloud/src/litellm-portal/{tenant-portal,ops-console}/hooks.ts` | §F.3: import the shared `extractError` (delete local copies; call sites unchanged) | Modify |
| `cloud/src/litellm-portal/components/panel-state.tsx` | §F.3: `PanelError` resolves the humanized message id via active i18n (also Task-2 file) | Modify |
| `cloud/src/litellm-portal/errors/error-boundary.tsx` (+`.test.tsx`) | §F.4: `BlockErrorBoundary` (per-block render isolation, LAYERED on MAJOR-1) | Create |
| `cloud/e2e/perf-budget.spec.ts` | §F.2: Playwright CLS<0.1 hard gate + LCP observed on `/`,`/usage`,`/ops` | Create |
| `cloud/playwright.config.ts` / `cloud/package.json` | §F.2: `test:e2e:perf` script + webServer if needed (read first) | Modify |
...
- **§F.2 perf gate** → **Task 3B** (§A.3 gate band, after Task 3). `cloud/e2e/perf-budget.spec.ts` is NEW-disjoint; `playwright.config.ts`/`package.json`/`DESIGN.md` SERIALIZE (touched by no other §F task except DESIGN.md). Runs in the §A GATE + Task-10 E2E.
- **§F.4 ErrorBoundary** → **Task 3C** (after Task 3 + Task 2C — consumes the §F.3 humane-message style; ADDITIVE on the closed MAJOR-1 try/catch, does NOT modify it). `errors/error-boundary.*` NEW-disjoint; `usage-charts-lazy.tsx`/`usage-dashboard.tsx`/`ops-console/screens/audit.tsx` SERIALIZE after Tasks 3/3B (chart files) & Task 2 (audit screen) — re-anchor by symbol.
- **§F.1 cockpit restyle** → **Task 6B** (§B band, after §A GATE + Tasks 2/2C/6 — consumes the Task-2 density tiers + Task-6 Ops shell). `components/density-toggle.*` NEW-disjoint; `ops-console/shell.tsx` SERIALIZES (Tasks 1/2/6/6B); the 4 Ops table screens are FILE-DISJOINT → **parallel wave**; `DESIGN.md` SERIALIZES. Tenant Portal NOT touched (Tenant stays comfortable).
- **§F.6 ops-theme token** → **Task 6C** (§B band; `ops-theme.ts`+`ops-theme.test.ts` touched by NO other task → fully file-disjoint, can run any time after §A GATE; `DESIGN.md` SERIALIZES).
- **Always-serialized shared files (unchanged rule, now incl. §F):** `DESIGN.md` (every §F task appends a section — strict task order), both shells, `branding.ts`, the i18n catalogs (`messages/{zh-CN,en}.ts` — §F.3 + §F.4 add keys; SERIALIZE with Task 9), `app.generated.ts`, `panel-state.tsx` (Task 2 + Task 2C), `usage-dashboard.tsx` (Tasks 1/3/3C).
- **§A GATE now includes §F hygiene items:** §F.5 (Task 1), §F.3 (Task 2C), §F.2 CLS (Task 3B) are §A-band and gate §B; §F.4 (3C), §F.1-cockpit (6B), §F.6 (6C) are §B-band (post-gate).
...
// `TenantPortalShellProps.brand` is NON-OPTIONAL (shell.tsx:21-26) — omitting
// it is `error TS2741: Property 'brand' is missing`. Copy the EXACT shape from
// routes/index.tsx:65-67 (verified live: `{ name: me.company, logoUrl: null,
// primaryColor: null }`) — do not guess.
...
> Keep `createTenantPortalRoutes` exported AND functionally intact (other code/tests may import it; do NOT delete it — it simply stops being the router's mount path). **`TenantPortalShellProps` (verified live, `shell.tsx:21-26`) requires `identity` AND `brand` (NON-optional) plus optional `impersonation?`; `PortalIndex` (`routes/index.tsx:42-49`) passes exactly `identity={toPortalIdentity(me)}`, `brand={toTenantBrand(me)}`, `impersonation={me.impersonation ?? null}`. The sample above mirrors that 1:1 (the `toTenantBrand` shape `{ name: me.company, logoUrl: null, primaryColor: null }` is copied verbatim from `routes/index.tsx:65-67` — re-read that file to confirm before coding; do not guess).** Backstop: if `PortalIndex` ever passes additional props, `TenantLayout` MUST include them derived from `me` identically — mirror PortalIndex 1:1. **Task-5 coupling:** Task 5 Step 1 instructs "do NOT pass brand/identity props directly — the shell is mounted by the route tree"; that ONLY holds because this `TenantLayout` passes `brand` (and `impersonation`) — without this fix Task 5's `/usage` render would be brand-less / TS-broken. The `id:"tenant-layout"` is new and unique (no collision with `ops-layout` or any `tenant-*-root` spec id — those are element `id`s, not route ids; route ids here are auto-derived from `path` for children + the explicit `"tenant-layout"` for the layout, exactly like `"ops-layout"`).
...
- [ ] **Step 5: Run → PASS** — `cd /Users/xumingyang/github/contrabass/cloud && bun run test src/litellm-portal/tenant-portal/routes.test.tsx` → PASS (2 tests: shell wraps `/usage`; `/` still via `indexRoute`). Then run the pre-existing router/index suite to prove no topology regression: `bun run test src/litellm-portal/router.test.tsx src/litellm-portal/index.test.ts` → PASS. **`hydration.test.tsx` at Task-0 Step-5 is STILL the pre-existing baseline: 6/7 pass, 1 fail = "tenant_admin full-nav shell at /" (the shipped Kumo-unseeded-random #418 — Task 0 does NOT touch that path and MUST NOT mask it; it is fixed in the NEXT task, Task 0B).** Acceptance here is narrower: run `bun run test src/litellm-portal/hydration.test.tsx` and confirm the ONLY failure is that exact pre-existing case (Task 0's pathless layout is identity-derived/SSR-safe exactly like `OpsLayout` — it introduces NO new #418 surface; the other 6 stay green, and the 1 red is unchanged from `d3e6b55`/`b46b998`). Task 0B then takes it to 7/7. The documented happy-dom `localhost:3000/kumo.css` flake (named exclusion, isolate-rerun-confirmed environmental) is also tolerated; nothing else. `bun run typecheck` → baseline-only.
...
## Task 0B (§A.0b — #418-safe shell skeleton, PRE-EXISTING-DEFECT FIX) [LOCKED-INVARIANT — RETAINED REVIEW PASS]

> **Sequence: Task 0 → Task 0B → Task 1** (§A chain). Task 0B depends on Task 0 (the failing render path is the Tenant shell, which Task 0 made route-mountable; the SSR-shell-at-`/` path is via `indexRoute`→`PortalIndex`, unchanged by Task 0, but Task 0B coordinates with Task 1's shell edits — see Files/serialization). **RETAINED-REVIEW (locked #418 invariant, same tier as Task 2C):** the reviewer MUST verify the SSR output of the fixed path contains NO `Math.random`-derived value and that the deterministic placeholder ⇄ real-SkeletonLine swap is post-hydration only (no `suppressHydrationWarning` band-aid).

**Why this exists (pre-existing shipped production #418 — authorized into Phase 3 by the user's decision rule):** `hydration.test.tsx` case **"tenant_admin full-nav shell at /"** (1 of the 7 `#418 guard` cases) is RED at `d3e6b55` (Phase-3 branch start = shipped Phase-1/2 `main`), at clean `b46b998`, and on the current tree — **PRE-EXISTING in production, NOT introduced by Phase 3 / Task 0 / Task 1** (verified empirically by the lead at all three points). It is the branch's namesake bug (`fix/litellm-portal-hydration-418`) latent in shipped code. **No CI workflow runs `cd cloud && bun run test`** (`ci.yml` Test = `make test-quick` = go + `packages/dashboard` + landing; `cloud-ci.yml` = build/deploy-dry; `portal-e2e.yml` = Playwright) — so the §A-GATE local vitest sweep is the ONLY gate this test gets. A carve-out is NOT acceptable; the gate must be genuinely green by fixing the root cause.

**Root cause (verified live):** the Tenant shell's loading state renders the `/` body `TenantOverviewScreen` (`PortalIndex`@`routes/index.tsx` renders `<TenantPortalShell><TenantOverviewScreen/></TenantPortalShell>`). `TenantOverviewScreen` (`tenant-portal/screens/overview.tsx`) renders Kumo `SkeletonLine` in its tile loading branches while `useDashboard().isLoading` (true on both SSR and client-first-render for this case — the dashboard query is NOT seeded). Kumo `SkeletonLine` (`node_modules/@cloudflare/kumo/dist/chunks/skeleton-line-*.js:5`) computes `--skeleton-width` / `--shimmer-duration` / `--shimmer-delay` from **UNSEEDED `Math.random()`** at render → SSR inline styles ≠ client first-render inline styles → genuine React #418 hydration mismatch. `src/test/setup.ts` (verified, 13 lines) does NOT seed `Math.random`; there is no SSR-stable Kumo SkeletonLine mode. **The failing SkeletonLines are in `overview.tsx`, NOT `shell.tsx` itself** (the shell renders the screen as `children`).

**Goal:** any shell loading state on the SSR path that renders Kumo `SkeletonLine` must be #418-safe — SSR renders a DETERMINISTIC stable placeholder (zero `Math.random` in server output); the real Kumo `SkeletonLine` (with its random shimmer) mounts CLIENT-ONLY, post-hydration. Same discipline family as Task 3's `warmUsageCharts`/loading-parity (SSR == client first render; dynamic/random bits mount after hydrate). **`suppressHydrationWarning` is FORBIDDEN as the fix** — it silences the warning without fixing the mismatch; the branch invariant demands a real deterministic-SSR + post-hydration-swap.

**Files:**
- Create: `cloud/src/litellm-portal/a11y/use-has-hydrated.ts` (tiny hook), `cloud/src/litellm-portal/a11y/use-has-hydrated.test.ts`
- Create: `cloud/src/litellm-portal/components/ssr-safe-skeleton.tsx` (the deterministic-SSR wrapper around Kumo `SkeletonLine`), `cloud/src/litellm-portal/components/ssr-safe-skeleton.test.tsx`
- Modify: `cloud/src/litellm-portal/tenant-portal/screens/overview.tsx` (replace the SSR-path `SkeletonLine` loading branches with the SSR-safe wrapper — the exact branches Step 1 pinpoints)
- **PRIMARY acceptance:** `cloud/src/litellm-portal/hydration.test.tsx` → 7/7 green (the previously-RED "tenant_admin full-nav shell at /" now PASSES; the other 6 stay green). NOT modified — it is the unchanged guard.
- **Serialization:** Task 0B precedes Task 1; Task 1 also edits `tenant-portal/shell.tsx` (NOT `overview.tsx`) — disjoint files, but both are in the shell-serialization family (Tasks 0/0B/1/2/5/6/7 touch shell/screen sources in order). Task 1 re-anchors by symbol as already stated. **Task 2 coupling:** Task 2 converges all screen `SkeletonLine` usages into a shared `PanelSkeleton` (`components/panel-state.tsx`). Task 0B's `SsrSafeSkeleton` is the deterministic-SSR primitive; **Task 2's `PanelSkeleton` MUST be built ON `SsrSafeSkeleton`** (note added in Task 2 — see plumbing) so the #418-safety is inherited, not lost, when overview migrates to `PanelSkeleton`. Task 0B stands alone (it precedes Task 2 and fixes the shell's own screen skeletons now).

- [ ] **Step 1: Investigate + document the exact SSR-path SkeletonLine renders (do NOT broad-refactor).** Read & record in the task notes: `hydration.test.tsx` lines ~244-340 (the `TASK_1W_CASES` table + the parametrized `it`, and the `name: "tenant_admin full-nav shell at /"` case — capture its exact failing `hydrationErrors` filter/output), `routes/index.tsx` `PortalIndex` (renders `<TenantPortalShell><TenantOverviewScreen/></TenantPortalShell>` for resolved `me`), `tenant-portal/screens/overview.tsx` (the three+ `if (isLoading) return (… <SkeletonLine …/> …)` tile branches — verified at ~`overview.tsx:64-68` `PersonalSpendTile`, `~113-117` `TeamBudgetTile`, `~167-174` `WebhookStatusTile`, and any other `useDashboard().isLoading`/`useTenantWebhook().isLoading` skeleton branch on the `/` render for a `tenant_admin` `me`), and `node_modules/@cloudflare/kumo/dist/chunks/skeleton-line-*.js:5` (the `Math.random()` `--skeleton-width`/shimmer computation). Enumerate the EXACT SkeletonLine call sites that are on the SSR render path of the failing case (the ones reached when `me` = tenant_admin and the dashboard/webhook queries are unseeded → `isLoading` true). The fix targets ONLY those; do not touch unrelated skeletons.

- [ ] **Step 2: Capture the RED baseline.** `cd /Users/xumingyang/github/contrabass/cloud && bun run test src/litellm-portal/hydration.test.tsx` → record: 6 pass, **1 fail = "server and client first render are identical: tenant_admin full-nav shell at /"** with a hydration-mismatch on a `style` attribute containing `--skeleton-width`/`--shimmer-*` (different SSR vs client values). This is the pre-existing defect — DO NOT modify the test; it is the acceptance oracle.

- [ ] **Step 3: Create the `useHasHydrated` hook** — `cloud/src/litellm-portal/a11y/use-has-hydrated.ts`. Standard SSR-safe mounted-gate: `false` on the server AND on the client's first (hydration) render, flips `true` in `useEffect` (post-hydration). This is the idiomatic React #418-safe pattern and is consistent with how `client.tsx`/Task-3 defer dynamic work until after `hydrateRoot` (post-hydration `useEffect`/warm-up). Ship VERBATIM:
...
- [ ] **Step 4: Failing hook test** — `cloud/src/litellm-portal/a11y/use-has-hydrated.test.ts` (`@vitest-environment happy-dom`): assert it returns `false` on the initial render and `true` after `act`/effects flush (mirror the existing happy-dom test idiom in the repo, e.g. `a11y/focus.test.ts`'s shape for imports). Run → it should PASS once Step 3 lands (write test first, run → FAIL "module missing", then Step 3 makes it green — standard TDD order; the PRIMARY oracle remains `hydration.test.tsx`).

- [ ] **Step 5: Create `SsrSafeSkeleton`** — `cloud/src/litellm-portal/components/ssr-safe-skeleton.tsx`. Renders a DETERMINISTIC, `Math.random`-free placeholder (`<div>` with stable Tailwind classes mirroring the skeleton's box geometry — fixed width via a deterministic class from the same `minWidth/blockHeight` props, NO Kumo `SkeletonLine`, NO inline random style) until `useHasHydrated()` is true, then renders the real Kumo `SkeletonLine` (its random shimmer is now client-only, post-hydration → never in SSR output, never in the client's first render → byte-identical SSR vs first-render → no #418). Ship VERBATIM (adjust the deterministic box classes to match the real `SkeletonLine` visual footprint — keep it a plain styled `<div>`):
...
> The placeholder `style` here is DETERMINISTIC (literal prop values, no `Math.random`) so it IS hydration-stable (same on SSR and client-first-render). The only non-deterministic thing — Kumo's random shimmer — is now exclusively post-hydration (client-only `hydrated===true`), which never participates in the hydration diff. This is the same shape as Task-3's "dynamic work after hydrate" discipline. (`bg-kumo-recessed` mirrors the Kumo skeleton surface — adjust to the exact token Kumo `SkeletonLine` uses if Step-1 inspection shows a different surface; the contract is "deterministic, random-free, geometry from props".)

- [ ] **Step 6: Failing component test** — `cloud/src/litellm-portal/components/ssr-safe-skeleton.test.tsx` (`@vitest-environment happy-dom`): (a) first render (pre-effect) shows `[data-ssr-skeleton-placeholder]`, NOT a Kumo SkeletonLine; (b) after effects flush it shows the real SkeletonLine; (c) the placeholder element has NO `Math.random`-derived inline style (assert its `style.width`/`height` equal the literal props). TDD: write → run → FAIL (module missing) → Step 5 → PASS.

- [ ] **Step 7: Replace the pinpointed SSR-path SkeletonLine renders in `overview.tsx`.** In `cloud/src/litellm-portal/tenant-portal/screens/overview.tsx`, for ONLY the loading branches Step 1 identified as on the failing case's SSR path (the `useDashboard().isLoading`/`useTenantWebhook().isLoading` tile branches reached at `/` for a `tenant_admin` `me` — verified ~`overview.tsx:64-68/113-117/167-174`): replace `import { SkeletonLine } from "@cloudflare/kumo/components/loader";` usage in those branches with `import { SsrSafeSkeleton } from "../../components/ssr-safe-skeleton";` and swap `<SkeletonLine minWidth=… maxWidth=… blockHeight=… />` → `<SsrSafeSkeleton minWidth=… maxWidth=… blockHeight=… />` (same props, byte-identical visual post-hydration). Do NOT alter non-SSR-path skeletons or other screens (Task 2 owns the broad `Panel*` convergence). Re-anchor by symbol — Task 0 did NOT edit `overview.tsx`; this is the first Phase-3 edit to it. If a branch's only `SkeletonLine` import becomes unused, remove the dead import; if still used elsewhere in the file by a non-SSR-path branch, keep it.

- [ ] **Step 8: Run the PRIMARY acceptance → 7/7 green.** `cd /Users/xumingyang/github/contrabass/cloud && bun run test src/litellm-portal/hydration.test.tsx` → **ALL 7 `#418 guard` cases PASS** (the previously-RED "tenant_admin full-nav shell at /" now passes — SSR & client first render emit the deterministic `[data-ssr-skeleton-placeholder]`, no `--skeleton-width`/`--shimmer-*` random style in the SSR output; the other 6 cases unchanged → still green). Also `bun run test src/litellm-portal/a11y/use-has-hydrated.test.ts src/litellm-portal/components/ssr-safe-skeleton.test.tsx` → PASS. CRITICAL: if any of the other 6 hydration cases regress, the fix touched too much — narrow it to the Step-1 pinpointed branches only. `bun run typecheck` → baseline-only.

- [ ] **Step 9: Commit**
```bash
cd /Users/xumingyang/github/contrabass
git add cloud/src/litellm-portal/a11y/use-has-hydrated.ts cloud/src/litellm-portal/a11y/use-has-hydrated.test.ts cloud/src/litellm-portal/components/ssr-safe-skeleton.tsx cloud/src/litellm-portal/components/ssr-safe-skeleton.test.tsx cloud/src/litellm-portal/tenant-portal/screens/overview.tsx
git -c commit.gpgsign=false commit -m "fix(litellm-portal): SSR-stable shell skeleton — kill pre-existing Kumo-random #418"
```

> **Task 1 / Task 2 coordination:** Task 1 edits `tenant-portal/shell.tsx` (NOT `overview.tsx`) — disjoint, but both serialize in the shell family (Task 0B before Task 1). Task 2's `PanelSkeleton` (`components/panel-state.tsx`) MUST be built on `SsrSafeSkeleton` (not raw Kumo `SkeletonLine`) so the #418-safety survives the §A.2 convergence — this coupling is noted in Task 2 (plumbing).
...
>
> **Steps 1–5 are ALREADY DONE (lead's in-flight implementer, green): `a11y/focus.ts`+test (2/2), `branding.ts` re-exports, and the shells' `useRouterState`/`FOCUS_RING`/`aria-current` edits applied. The edits below (Step 6/7 harness fix + the NEW bare-test migration Step 9c) are COMPATIBLE with those committed-pending edits — they only add the missing `await router.load()` to the new harness and migrate the 8 pre-existing bare-shell `it`s across 3 files (incl. `shell-impersonation.test.tsx`); Steps 1–5 are NOT re-touched.**
>
...
- [ ] **Step 9c (CRITICAL — MIGRATE the pre-existing BARE-shell tests; without this Step 10 + Step 17 cannot be green and the §A GATE goes RED, hard-blocking §B/§C).** Step 9's unconditional `useRouterState` (now the FIRST hook in BOTH shells, including before `OpsConsoleShell`'s Owner guard) makes ANY render WITHOUT a Router context throw `TypeError: Cannot read properties of undefined (reading 'isServer')` (TanStack reads router state from context). The blast radius is **8 `it`s across 3 files** (NOT 5/2 — the original census missed `shell-impersonation.test.tsx`). They MUST be migrated to the production-router harness (the same `await router.load()` harness used by `renderTenantAt`/`renderOpsAt` in Steps 6/7 — and by the Phase-2-proven `ops-console/routes.test.tsx:58-74`).

  **CENSUS DISCIPLINE (do NOT trust this list's count — reconcile against the LIVE tree):** before migrating, run from `cloud/`: `rg -l 'TenantPortalShell|OpsConsoleShell' src/litellm-portal --glob '*test*'` and, for EACH hit, inspect its render helper — migrate EVERY one that renders the shell **bare** (no Router/`RouterProvider`/`createPortalRouter` in its wrapper). Files that already drive the shell through the production router (e.g. `hydration.test.tsx`, `router.test.tsx`, the new `tenant-portal/routes.test.tsx`) or only reference the symbol in non-rendering ways (e.g. `ops-console/i18n-completeness.test.ts`) need NO migration. The 8 (verified live 2026-05-17 — reconcile, do not assume):
  - `cloud/src/litellm-portal/tenant-portal/shell.test.tsx` — 3 `it`s under `describe("TenantPortalShell", …)` (`renderWithI18n`, no router): "shows all six nav items for a tenant_admin", "shows only Overview/Usage/API Key for a member", "shows a Phase-2 notice and a /manage link for a pure Owner".
  - `cloud/src/litellm-portal/ops-console/shell.test.tsx` — 2 `it`s under `describe("OpsConsoleShell", …)` (`renderShell`, no router): "Owner sees all seven nav items + ops chip", "non-Owner sees a 403 card + link to / (no nav)".
  - `cloud/src/litellm-portal/tenant-portal/shell-impersonation.test.tsx` — **3 `it`s** under `describe("TenantPortalShell impersonation banner", …)` whose `renderWithI18n` (helper @~lines 20-25) wraps ONLY `QueryClientProvider`+`I18nProvider` (NO Router) and renders `<TenantPortalShell …>` bare (@~lines 45/61/70): "renders an ARIA-alert banner with an exit control when impersonating", "renders no impersonation alert banner when impersonation is null", "renders no impersonation alert banner when impersonation is omitted". (This file also has `vi.mock("@cloudflare/kumo/components/toast", …)` for `Toasty` @line 7 and `vi.mock("../ops-console/hooks", …)` for `useStopImpersonation` @line 8 — **PRESERVE BOTH mocks verbatim**; they are unrelated to the router and the impersonation banner's exit button depends on the `useStopImpersonation` mock.)

  Migration rule: replace the bare `renderWithI18n(<TenantPortalShell …>)` / `renderShell(<OpsConsoleShell …>)` calls with the SAME-FILE async router harness (`renderTenantAt(path)` / `renderOpsAt(path)` from Steps 6/7 — reuse; do NOT add a second harness; for `shell-impersonation.test.tsx`, add a `renderTenantAt`-shaped helper local to that file mirroring Step 6's exactly, keeping the file's two `vi.mock`s). Because the shells render via the route tree (Task 0's `tenantLayoutRoute` / Phase-2's `opsLayoutRoute`), drive by PATH + seeded `me`, not by passing the shell element directly:
  - "six nav items for tenant_admin" → `await renderTenantAt("/usage")` with `adminMe` (`tenantRole:"tenant_admin"`); assert the 6 nav links present (query within `#tenant-portal-shell-root`). Identity comes from the seeded `ME_QUERY_KEY` via `TenantLayout` (Task 0).
  - "member sees only 3" / "pure-Owner Phase-2 notice + /manage link" → seed the respective `me` (member: `tenantRole:"member"`; pure Owner: `role:"admin",tenantRole:null,tenantTeamId:null`) and `await renderTenantAt("/")` (the pure-Owner Phase-2 notice path is owned by `indexRoute`→`PortalIndex`; route to `/` for that case — Task 0 left `/` with `indexRoute`). Same expectations (3 nav links / Phase-2 notice / `/manage` link).
  - "Owner sees seven nav items + ops chip" → `await renderOpsAt("/ops")` with `ownerMe`; assert nav + chip inside `#ops-console-shell-root`.
  - "non-Owner sees 403 + link to /" → seed a non-Owner `me` (`role:"user"`/no tenant-admin), `await renderOpsAt("/ops")`; assert the 403 card. (`OpsLayout`/`OpsConsoleShell` render the forbidden card for non-Owner — Phase-2 behavior, unchanged; `useRouterState` is unconditional-before-the-guard per Step 9 so hook order is stable in both branches.)
  - **`shell-impersonation.test.tsx` (3 `it`s) — migrate via `renderTenantAt("/usage")` with seeded `me.impersonation`** (RESOLVED OQ: `MeSchema` HAS `impersonation: z.object({realActor,effectiveTeamId}).nullable().optional()` @`schemas.ts:15-28`, so it is fully seedable through the hydrated `ME_QUERY_KEY` — `TenantLayout` passes `impersonation={me.impersonation ?? null}` exactly like `PortalIndex`@`routes/index.tsx:44`):
    - "renders an ARIA-alert banner with an exit control when impersonating" → seed `me = { …tenant_admin me…, impersonation: { realActor: "real@x.com", effectiveTeamId: "t1" } }`, `await renderTenantAt("/usage")`; assert the ARIA-alert banner + exit control render (same assertions as the bare test; the `useStopImpersonation` mock still backs the exit button).
    - "renders no impersonation alert banner when impersonation is null" → seed `me.impersonation = null`, `await renderTenantAt("/usage")`; assert NO banner.
    - "renders no impersonation alert banner when impersonation is omitted" → seed `me` WITHOUT the `impersonation` key (omit it), `await renderTenantAt("/usage")`; assert NO banner (the `?? null` coalesce yields no banner). NO harness-limitation fallback — all 3 migrate cleanly through the router with seeded `me`.
  Delete the now-unused bare `renderWithI18n`/`renderShell` helpers IF nothing else in that file uses them (grep within the file first; if other untouched `it`s use them, keep the helper but the migrated `it`s use the router harness). Each migrated `it` becomes `async`, `await`s the harness, uses `findBy*`/`waitFor` for the post-`router.load()` async mount (mirror Steps 6/7). **Do NOT weaken any assertion** — same expected nav counts / 403 / Phase-2-notice / banner-present-or-absent; only the render mechanism changes (bare → production-router). This also retro-fixes the latent defect that these bare tests never exercised the real mount path.

- [ ] **Step 10: Run → PASS** — `cd /Users/xumingyang/github/contrabass/cloud && bun run test src/litellm-portal/tenant-portal/shell.test.tsx src/litellm-portal/ops-console/shell.test.tsx src/litellm-portal/tenant-portal/shell-impersonation.test.tsx` → PASS = **all 8 MIGRATED pre-existing `it`s across the 3 files (Step 9c) GREEN + the new Step 6/7 a11y `it`s GREEN** (no `isServer` TypeError anywhere — every shell render now goes through a router via the Task-0 `tenantLayoutRoute` / Phase-2 `opsLayoutRoute`). If ANY bare `it` still throws `isServer`, Step 9c was incomplete (re-run the `rg` census) — finish the migration, do NOT skip/relax. `bun run typecheck` → baseline-only.
...
const trendAriaLabel = buildChartAriaDescription(
  // windowLabel: the `win` state IS the DashboardWindow ("24h"|"48h"|"7d"
  //   |"30d"); WindowSelector (components/panel.tsx) renders that raw value
  //   AS its option label (no separate label map exists — verified). Reuse
  //   that SAME source: pass the `win` state value as the window label.
  // grain: data.grain — VALID (DashboardResponseSchema.grain = "hour"|"day").
  //   There is NO data.windowLabel field (verified dashboard-schemas.ts).
...
> Anchor note (corrected — verified live `dashboard/dashboard-schemas.ts:17-42`): `DashboardResponseSchema` has **NO `windowLabel`** field. Its real fields: `available, empty, scope, window, grain, grainFallback, timezone, kpi, trend, models, perUser, summary`. **Window label source:** the `win` component state (`usage-dashboard.tsx:~97,113`, type `DashboardWindow`) is what `WindowSelector` (`usage-dashboard.tsx:~144`, defined in `dashboard/components/panel.tsx`) renders AS its option labels — `panel.tsx` iterates `DASHBOARD_WINDOWS` and renders the raw `windowValue` string (e.g. `"30d"`) directly as the label; there is NO separate window→label map. So pass `win` as `windowLabel` — that IS the same label the user sees in the selector. Before coding, READ `dashboard/components/panel.tsx`'s `WindowSelector` to confirm it still renders the raw window value (if a label map is ever added there, reuse THAT map — do not invent one). **`grain` IS valid** (`data.grain`, `z.enum(["hour","day"])`); keep `grain: data?.grain ?? ""`. `points` MUST be `trendSeries[0]?.points.length ?? 0`. The `t` macro string `Token 用量趋势图，时间范围为 {window}，共 {points} 个 {grain} 粒度数据点。` becomes a catalog key (added to BOTH catalogs in Task 9; English: `"Token usage trend chart, range {window}, {points} {grain}-grain data points."`). The macro runs in `usage-dashboard.tsx` (inside `I18nProvider`), keeping `trend-chart.tsx` macro-free.
...
- [ ] **Step 17: Run the §A.1 regression sweep** — `cd /Users/xumingyang/github/contrabass/cloud && bun run test src/litellm-portal/tenant-portal/shell.test.tsx src/litellm-portal/tenant-portal/shell-impersonation.test.tsx src/litellm-portal/ops-console/shell.test.tsx src/litellm-portal/a11y/focus.test.ts src/litellm-portal/dashboard/charts/build-chart-aria.test.ts src/litellm-portal/dashboard/charts/trend-chart.test.tsx` → all PASS. CRITICAL non-regression: `shell-impersonation.test.tsx`'s 3 `it`s — MIGRATED in Step 9c to the production-router harness with seeded `me.impersonation` (set/null/omitted) — stay green; the impersonation `role="alert"` banner behavior is unchanged (only the render mechanism became router-driven; the `Toasty` + `useStopImpersonation` `vi.mock`s preserved). `bun run typecheck` → baseline-only.

### §F.5 type-scale audit (V2.0 §1.1 fold-in — same §A.1 hygiene band)

- [ ] **Step 18: Write the failing type-scale audit test** — `cloud/src/litellm-portal/a11y/type-scale.test.ts`. It reads the REAL token source (`DESIGN.md` Typography Hierarchy table) and asserts ≤7 tiers + heading:body ratio ≤ 2× (V2.0 §1.1: token-ized type scale ≤7 tiers, heading 1.5–2× body). The current DESIGN.md has **8 tiers** and ratio **2.14–2.25×** (verified) — this test FAILS until Step 19 converges it.
...
- [ ] **Step 19: Run → FAIL, then converge the DESIGN.md type scale to ≤7 tiers + ≤2× (Kumo-internal, no token-override).** `cd /Users/xumingyang/github/contrabass/cloud && bun run test src/litellm-portal/a11y/type-scale.test.ts` → FAIL (8 tiers; 36/16 = 2.25×). Converge by EDITING the `DESIGN.md` Typography `### Hierarchy` table (the token-source contract — this is a doc/usage convergence, NOT a Kumo fork or a versh-token override; §B.0 forbids overriding Kumo's tokens, it does not forbid the portal documenting which Kumo `Text` variants it uses):
  - **Merge the two 12px/600/uppercase/tracking-wider rows** — `Metric label` and `Table header` are byte-identical in size/weight/transform; collapse into ONE tier `Label (12px / 600 / uppercase / tracking-wider — metric labels & table headers)`. (8 → 7 tiers.)
  - **Cap Page title at ≤ 2× body**: change the `Page title` row from `30px → 36px desktop` to `28px → 32px desktop` (32/16 = 2.0× desktop, 28/14 = 2.0× mobile — exactly the V2.0 ceiling; this also dedups with `Metric value` 28px? NO — keep `Metric value` 28px mono distinct by role; the audit tier-count keys off the table rows, and Page title 28→32 vs Metric value 28 remain two semantically distinct rows = still ≤7 after the label merge). Update the matching `DESIGN.md` `## Typography → ### Principles` line and any `tracking-tight` page-title note to the new sizes. NO Kumo `Text` variant is forked — the page-title element uses Tailwind size utilities already; this changes the documented/used size, the locked §B.0 "标题权重 ≤ 600 / tracking-tight" rule is preserved (weight unchanged, only px lowered).
  - **Document the real ratios** in a new DESIGN.md note under `### Principles`: "Type scale: 7 tiers. Largest heading (Page title 32px desktop) : body (16px) = 2.0× — at the V2.0 §1.1 ceiling, intentional for an institutional ops dashboard." If after the merge the test still computes >7 or >2.0×, the executor MUST further merge `Caption (13px)` into `Body (14px)` (13→14 is visually negligible; Caption uses are meta/timestamps — acceptable) and re-run; do NOT weaken the test, converge the scale. Re-run Step-18 test → PASS.

- [ ] **Step 20: Add the 5-second-test + blur-test to the §A.1 manual acceptance (V2.0 §1.1 verification, human-judgment).** Append to the Task-10 Step-2 manual staging checklist (item-1 a11y) the procedure + pass bar (this is a human acceptance, not an automatable assertion — specified concretely so it is not hand-waved):
  - **5-second test**: screenshot Tenant `/` (overview) and Ops `/ops` (tenant-overview), light AND dark. Show each to a reviewer for exactly 5s, then hide. Reviewer must, from memory, name (a) the page's primary CTA/action and (b) the main heading. **Pass bar: reviewer correctly names BOTH for all 4 screenshots.** Fail → visual hierarchy not strong enough (revisit §B.1/§B.2 emphasis, NOT the type scale alone).
  ...
  - These are recorded as Task-10 Step-2 sub-items (cross-referenced there); no code change in Task 1.

- [ ] **Step 21: Commit**
```bash
cd /Users/xumingyang/github/contrabass
git add cloud/src/litellm-portal/a11y cloud/src/litellm-portal/tenant-portal/branding.ts cloud/src/litellm-portal/dashboard/charts/build-chart-aria.ts cloud/src/litellm-portal/dashboard/charts/build-chart-aria.test.ts cloud/src/litellm-portal/dashboard/charts/trend-chart.tsx cloud/src/litellm-portal/dashboard/charts/trend-chart.test.tsx cloud/src/litellm-portal/dashboard/views/usage-dashboard.tsx cloud/src/litellm-portal/tenant-portal/shell.tsx cloud/src/litellm-portal/tenant-portal/shell.test.tsx cloud/src/litellm-portal/ops-console/shell.tsx cloud/src/litellm-portal/ops-console/shell.test.tsx cloud/src/litellm-portal/a11y/type-scale.test.ts cloud/src/litellm-portal/DESIGN.md
git -c commit.gpgsign=false commit -m "feat(litellm-portal): a11y pass — focus ring, aria-current, chart aria, type-scale audit"
```
...
- [ ] **Step 9: Wrap each shell in `DensityProvider` (shell default = §A.2; preference override = §F.1, wired in Step 9b).** In `cloud/src/litellm-portal/tenant-portal/shell.tsx` import `import { DensityProvider, resolveDensity } from "../components/density";` and wrap the returned shell root's children (anchor: the `<div id="tenant-portal-shell-root" …>` opening@148) with `<DensityProvider density={resolveDensity(densityPref, "comfortable")}>…</DensityProvider>` (place the provider as the first child inside `#tenant-portal-shell-root`, wrapping everything from `{impersonation ? … }` through `</main>`/the nav `<div>`). In `cloud/src/litellm-portal/ops-console/shell.tsx` do the same with shell default `"compact"`: `<DensityProvider density={resolveDensity(densityPref, "compact")}>` wrapping the Owner branch's chrome (the non-Owner 403 branch needs no provider). `densityPref` is the hydrated preference threaded in Step 9b (until Step 9b lands, pass `undefined` so `resolveDensity` returns the shell default — i.e. Step 9 alone is byte-equivalent to the old hardcoded `density="comfortable"`/`"compact"`; Step 9b adds the override). Keep all existing structure; only add the provider wrapper. (`density.test.tsx`'s provider/`densityClasses`/`resolveDensity` cases from Step 5 cover the primitive; the shell wrap is structural.)

- [ ] **Step 9b: Thread the persisted density preference (§F.1, V2.0 §1.2 — reuse the existing preferences channel; SSR/#418-safe).** The portal already has `usePreferences`/`useUpdatePreferences` (`hooks/use-preferences`, used by `routes/__root.tsx` `HeaderActions` for the theme toggle) backed by `UserPreferences` (`schemas.ts`) and the `/api/preferences` PUT, SSR-seeded via `PREFERENCES_QUERY_KEY` (verified: `server-impl.tsx`:79 `serverQueryClient.setQueryData(PREFERENCES_QUERY_KEY, preferences)`). Extend it minimally — additive, optional, old-client-safe:
  - In `cloud/src/litellm-portal/schemas.ts`, the `UserPreferences`/preferences Zod schema: add `density: z.enum(["comfortable", "compact"]).optional()` (additive optional — existing cached prefs without it stay valid; mirrors how `theme` is modeled — read the file for the exact schema name & shape and match it; do NOT invent a new schema).
  - In each shell, read it: `const densityPref = usePreferences().data?.density;` (import `usePreferences` from `../hooks/use-preferences` — already a dependency pattern in `__root.tsx`). Pass `densityPref` into the Step-9 `resolveDensity(densityPref, <shellDefault>)`. Because preferences are SSR-seeded into the same hydrated QueryClient (`PREFERENCES_QUERY_KEY`), server and client first-render resolve the IDENTICAL density → no #418 (same channel/discipline as the theme toggle, which is already #418-safe per `hydration.test.tsx`).
  - **Toggle UI is NOT here** (this step only makes density preference-driven + SSR-safe). The actual comfortable↔compact toggle control + `useUpdatePreferences().mutate({ density })` wiring is **Task 6B** (§B restyle band — Ops chrome), so the hygiene §A.2 step stays minimal/low-risk and the visible control lands with the cockpit restyle.
  - Test: extend `cloud/src/litellm-portal/components/density.test.tsx` (already has `resolveDensity` cases from Step 5) — no new file. Add a `hydration.test.tsx` parity NOTE only (do NOT add a heavy new case here): the existing Task-3 Step-9 parity cases already render `/` and `/ops` via the real SSR→hydrate harness with the seeded QueryClient; since `density` rides the SAME `PREFERENCES_QUERY_KEY` seed as `theme` (already covered), a preference-driven density is structurally identical to the already-#418-proven theme path — Task 9 finalization re-runs `hydration.test.tsx` and that coverage is sufficient (documented here so the executor does not duplicate the harness).
...
```bash
cd /Users/xumingyang/github/contrabass
git add cloud/src/litellm-portal/components cloud/src/litellm-portal/tenant-portal/screens cloud/src/litellm-portal/ops-console/screens cloud/src/litellm-portal/tenant-portal/shell.tsx cloud/src/litellm-portal/ops-console/shell.tsx cloud/src/litellm-portal/schemas.ts cloud/src/litellm-portal/DESIGN.md
git -c commit.gpgsign=false commit -m "feat(litellm-portal): unify panel state + density tokens (incl §F.1 table tier + persisted pref)"
```

---

## Task 2C (§A hygiene — §F.3 V2.0 §2.4 + §4#6): error-code → human 3-element message map [SECURITY/CONTRACT-SENSITIVE — RETAINED REVIEW PASS]

> **MARKED FOR THE RETAINED SECURITY/DESIGN REVIEW PASS** (same tier as the §C `branding.ts` task). Server error codes currently leak raw into Banners; this task introduces the localized 3-element error contract at the shared `extractError` boundary. The reviewer MUST verify: no internal implementation string leaks (DO names, paths, stack, snake_case codes, HTTP numbers), the unknown-code fallback never echoes the raw code, and the recovery-action wording for permission codes (`impersonation_required`) does not leak auth internals.
>
> §A-hygiene placement (low-risk, file-disjoint-ish): this is a pure mapping + a 2-file `extractError` unification — no shell/chrome/branding coupling — so it slots in the §A band after Task 2 (it depends on `PanelError` from Task 2 for the render path it feeds). It does NOT gate §B (the §A GATE covers Tasks 1/2/2C/3 hygiene).

**Files:** Create `cloud/src/litellm-portal/errors/error-messages.ts`, `cloud/src/litellm-portal/errors/error-messages.test.ts`; modify `cloud/src/litellm-portal/tenant-portal/hooks.ts` + `cloud/src/litellm-portal/ops-console/hooks.ts` (unify `extractError`), `cloud/src/litellm-portal/components/panel-state.tsx` (`PanelError` uses the humanized message), `cloud/src/litellm-portal/i18n/messages/{zh-CN,en}.ts`.

- [ ] **Step 1: Enumerate the real server error codes** — `cd /Users/xumingyang/github/contrabass/cloud && rg -no 'error: "[a-z_]+"' src/litellm-portal/routes.ts | sed 's/.*"\(.*\)"/\1/' | sort -u`. Verified non-exhaustive set (2026-05-17, the executor MUST re-run and reconcile): `admin_required`, `owner_required`, `impersonation_required`, `email_required`, `invalid_json`, `invalid_body`, `unauthorized`, `not_found`, `validation_error`, `key_id_required`, `key_not_found`, `team_id_required`, `team_not_found`, `user_id_required`, `user_not_found`, `confirm_alias_required`, `confirm_alias_mismatch`, `confirm_email_mismatch`, `litellm_team_create_failed`, `index_do_unavailable`, `team_config_do_unavailable`, `invite_already_consumed`, `invite_not_found`, `team_and_user_required`, `billing_archive_unavailable`, `invalid_period`, `billing_archive_not_found`, `no_tenant_scope`, plus the client-side `*_request_failed`/`*_failed` fallbacks in `tenant-portal/hooks.ts` + `ops-console/hooks.ts`. The map MUST cover every code reachable by a portal request (Tenant + Ops surfaces); codes only reachable by non-portal admin APIs may map to the generic fallback.

- [ ] **Step 2: Write the failing error-message test** — `cloud/src/litellm-portal/errors/error-messages.test.ts`:
...
- [ ] **Step 3: Run → FAIL** — `cd /Users/xumingyang/github/contrabass/cloud && bun run test src/litellm-portal/errors/error-messages.test.ts` → FAIL (module missing).

- [ ] **Step 4: Implement `cloud/src/litellm-portal/errors/error-messages.ts`** — the single source of truth. Each entry is a Lingui zh source-string message id (3 elements folded into one human sentence: what happened + input-preserved cue when applicable + recovery action). The component renders it via the active i18n (zh/en parity added to catalogs in Step 6 + Task 9):
...
> The executor MUST reconcile `MAP`'s keys with Step-1's live `rg` enumeration: every portal-reachable code present → mapped (add any missing with a 3-element sentence following the established pattern); the `KNOWN_ERROR_CODES.length` floor in the test pins drift. Wording: imperative recovery action, never the code, never `_`. The `confirm_email_required` key is included even if not in the verified dump (paired with `confirm_email_mismatch`) — drop it only if `rg` proves it unreachable.

- [ ] **Step 5: Unify `extractError` to route through `errorMessage` (both hooks files).** Today `tenant-portal/hooks.ts`@28 and `ops-console/hooks.ts`@31 each have a private `extractError(json, fallback): string` returning the raw code (or fallback code). Replace BOTH with a shared import so the humanization is single-sourced and cannot drift:
  - Create `cloud/src/litellm-portal/errors/extract-error.ts`:
...
```
  ...
  - In `tenant-portal/hooks.ts`: delete the local `function extractError(...)`@~28-34 and add `import { extractError } from "../errors/extract-error";`. The ~9 call sites (`throw new Error(extractError(json, "tenant_*_failed"))`) are UNCHANGED — same signature `(json, fallbackCode) => string`; the returned string is now a human message id instead of a raw code. The `*_failed` fallback codes are NOT in `MAP` → they resolve to `GENERIC_FALLBACK_ID` (a humanized safe fallback) — strictly better than today's raw `tenant_invites_request_failed` in a Banner.
  - In `ops-console/hooks.ts`: delete the local `function extractError(...)`@~31-37, add the same import; its `getJson` + mutation call sites are UNCHANGED (same signature). (`ops_*_failed` fallbacks → generic fallback, same improvement.)
  - **Type/signature invariant**: `extractError(json: unknown, fallbackCode: string): string` is byte-identical to both removed locals → no call-site edits, no type drift. `i18n`: the returned string is a zh message id; the existing `PanelError`/`Banner` already render their `description`/`error.message` through the React tree under `I18nProvider`, so a `t`/catalog-resolved id renders localized. **BUT** `errorMessage` returns a raw zh string literal (a message id), not a `t`-macro call — for en to resolve it MUST be a catalog key. Step 6 adds every `MAP` value + `GENERIC_FALLBACK_ID` as a catalog key in BOTH `zh-CN.ts` (identity) and `en.ts` (English), and `PanelError` (next step) wraps the message via the i18n runtime (`i18n._(msg)`), consistent with how the catalog is consumed elsewhere.

- [ ] **Step 6: Make `PanelError` resolve the message through i18n + add catalog keys.** In `cloud/src/litellm-portal/components/panel-state.tsx` `PanelError` (from Task 2): its current shape is `const message = error instanceof Error ? error.message : t\`网络请求失败\``. Since `error.message` is now an `errorMessage()` id (a zh catalog key), resolve it through the active i18n so en renders English: change to use the Lingui runtime — `import { useLingui } from "@lingui/react";` then `const { i18n } = useLingui(); const raw = error instanceof Error ? error.message : ""; const message = raw ? i18n._(raw) : t\`网络请求失败\`;` (the `i18n._(id)` returns the localized string for the active locale; if the id is not a catalog key Lingui returns the id verbatim — which is already the human zh sentence, so still safe, never a raw code). Add EVERY `MAP` value string + `GENERIC_FALLBACK_ID` to BOTH `cloud/src/litellm-portal/i18n/messages/zh-CN.ts` (identity-mapped, as the catalog convention) and `cloud/src/litellm-portal/i18n/messages/en.ts` (English translations of each 3-element sentence — same 3-element structure in English: what happened · input preserved · next step). Task 9's i18n-completeness + the Phase-3 `PHASE3_KEYS` fixture MUST include these (cross-referenced in Task 9).

- [ ] **Step 7: Run → PASS** — `cd /Users/xumingyang/github/contrabass/cloud && bun run test src/litellm-portal/errors src/litellm-portal/tenant-portal/hooks.test.tsx src/litellm-portal/ops-console/hooks.test.tsx src/litellm-portal/components/panel-state.test.tsx` → PASS. CRITICAL non-regression: the existing hooks tests assert error-throwing behavior — confirm they still pass with the message now humanized (if a hooks test asserted the literal raw code string, that assertion must update to the human message id — note this is an intended behavior change: raw codes were a defect; update the assertion to the new contract, do NOT weaken it). `bun run typecheck` → baseline-only.

- [ ] **Step 8: `DESIGN.md` — add `## Error UX Contract (Phase 3 §F.3)`.** Insert after `## State Treatments`:
...
```bash
cd /Users/xumingyang/github/contrabass
git add cloud/src/litellm-portal/errors cloud/src/litellm-portal/tenant-portal/hooks.ts cloud/src/litellm-portal/ops-console/hooks.ts cloud/src/litellm-portal/components/panel-state.tsx cloud/src/litellm-portal/i18n/messages cloud/src/litellm-portal/DESIGN.md
git -c commit.gpgsign=false commit -m "feat(litellm-portal): humanize server error codes (3-element contract)"
```
...
> **§A GATE — §B may NOT begin until this passes.** Before any §B task: `cd /Users/xumingyang/github/contrabass/cloud && bun run test src/litellm-portal/tenant-portal/routes.test.tsx src/litellm-portal/a11y src/litellm-portal/components src/litellm-portal/errors src/litellm-portal/bundle-gate.test.ts src/litellm-portal/hydration.test.tsx src/litellm-portal/dashboard/charts/build-chart-aria.test.ts src/litellm-portal/dashboard/charts/trend-chart.test.tsx src/litellm-portal/tenant-portal/shell.test.tsx src/litellm-portal/tenant-portal/shell-impersonation.test.tsx src/litellm-portal/ops-console/shell.test.tsx` ALL green + the **§F.2 CLS hard gate** (`bun run test:e2e:perf` — Task 3B, CLS < 0.1 on `/`,`/usage`,`/ops`) green + `bun run typecheck` baseline-only. This is the §A acceptance gate (decision 3 / §D.2) — it now includes: the **Task-0 tenant route-topology guard (`tenant-portal/routes.test.tsx` — shell wraps `/usage`)**, **`hydration.test.tsx` 7/7 (Task 0B fixes the pre-existing shipped "tenant_admin full-nav shell at /" Kumo-random #418 so this is now genuinely satisfiable — at Task-0 it is still 6/7; from Task 0B onward the §A GATE requires 7/7)** plus the Task-3 NEW-C-A #418 parity + negative-control cases in the same file, the §F.3 error-contract tests (Task 2C), the §F.5 type-scale audit (Task 1), the §F.2 CLS perf gate (Task 3B), and the §F.4 ErrorBoundary tests (Task 3C). The 8 migrated bare-shell `it`s (Task 1 Step 9c) ride in `tenant-portal/shell.test.tsx` / `ops-console/shell.test.tsx` / `tenant-portal/shell-impersonation.test.tsx` — all three are in this sweep so the migration is GATED (not merely run ad-hoc in Step 17). §F + Task-0/0B additions are hygiene-band — they gate §B exactly as §A.1–§A.3 do. **NO CI backstop:** no workflow runs `cd cloud && bun run test`; THIS local sweep is authoritative and must be genuinely green except the two by-name isolated-confirmed exclusions (the kumo.css fetch flake; the pre-existing-on-`main` `?window=90d`) — never "CI covers it".

---

## Task 3B (§A.3 gate band — §F.2 V2.0 §2.1): Web-Vitals performance budget (CLS hard gate)

> Same §A.3 band as the ECharts-lazy gate. CLS < 0.1 is a CI HARD GATE (deterministic, assertable). INP/LCP are observed-and-recorded (not gates) — consistent with §A.3's "observed metric, no KB threshold" precedent. Reuses the EXISTING Playwright harness (`@playwright/test` + `playwright.config.ts` + `test:e2e` — verified present); NO Lighthouse/new dependency.

**Files:** Create `cloud/e2e/perf-budget.spec.ts`; modify `cloud/playwright.config.ts` (add a `perf` project if needed — read it first), `cloud/package.json` (add `test:e2e:perf` script), `cloud/src/litellm-portal/DESIGN.md` (observed-metrics record).

- [ ] **Step 1: Read the real Playwright setup** — `cd /Users/xumingyang/github/contrabass/cloud && cat playwright.config.ts && rg -n '"test:e2e"' package.json`. Determine: the `baseURL`/`webServer` (how the portal is served for E2E — local `wrangler dev`/`vite`/static), and the existing `testDir`/projects. The perf spec MUST drive the SAME server the existing `test:e2e` uses (do NOT invent a new server). If `playwright.config.ts` has no `webServer`/`baseURL` usable headless, specify the minimal addition (a `webServer` running the existing portal dev/preview command) — concretely, not hand-waved.

- [ ] **Step 2: Write the perf-budget spec** — `cloud/e2e/perf-budget.spec.ts`. CLS hard-asserted; LCP/INP collected + logged (informational). Uses in-page `PerformanceObserver` (web-perf primitives, no extra dep):
...
> INP cannot be reliably auto-measured without a scripted representative interaction; this spec measures CLS (hard) + LCP (observed). INP is a Task-10 manual staging item (Step 2 below adds it to the E2E checklist with a concrete procedure: open devtools Performance/INP overlay, perform one representative interaction per route, record). This is the honest scope — CLS is the deterministic hard gate (the locked §F.2 decision), LCP/INP observed.

- [ ] **Step 3: Add the `test:e2e:perf` script** — in `cloud/package.json` add `"test:e2e:perf": "playwright test e2e/perf-budget.spec.ts"` (or, if `playwright.config.ts` uses `testDir`, place the spec under that dir and script `playwright test --grep "§F.2 perf budget"`). The §A GATE (above) and Task-10 E2E run this; a CLS breach fails it (non-zero exit) → CI red. If `playwright.config.ts` needs a `webServer` to serve the portal, add the minimal one running the existing portal preview/dev command (read Step-1 output; do not invent).

- [ ] **Step 4: Run + record** — `cd /Users/xumingyang/github/contrabass/cloud && bun run test:e2e:perf`. If a route's CLS ≥ 0.1, that is a real defect to fix before §B (the §A.2 `PanelSkeleton` fixed-height contract is the primary CLS defense — verify skeleton heights match resolved content; do NOT relax the gate). Capture the printed `[perf] route=… CLS=… LCP=…` lines and append to `DESIGN.md` `### Chart / Bundle 决策` as a dated observational record (CLS hard-gate result + LCP observed values; INP from the Task-10 manual item). Commit:
```bash
cd /Users/xumingyang/github/contrabass
git add cloud/e2e/perf-budget.spec.ts cloud/playwright.config.ts cloud/package.json cloud/src/litellm-portal/DESIGN.md
git -c commit.gpgsign=false commit -m "feat(litellm-portal): CLS<0.1 perf hard gate + LCP observed (Playwright)"
```

---

## Task 3C (§A.3 / §F.4 V2.0 §2.4): per-block ErrorBoundary — LAYERED on the MAJOR-1 try/catch

> **DOES NOT touch the closed #418 §A.3 workstream.** The MAJOR-1 `client.tsx` `try { await warmUsageCharts() } catch {}` (chunk-fetch white-screen floor) is UNCHANGED. This task ADDS a React ErrorBoundary for RENDER/RUNTIME isolation. The two are orthogonal and explicitly layered: (a) try/catch = "can't fetch the chunk" floor (stays); (b) ErrorBoundary = "the rendered chart/audit/dashboard block threw" isolation. Neither swallows the other; the §E-1 loading-parity test, the `not.toContain` §A-gate pins, the warm-up, and OQ-(b) reasoning are NOT modified.

**Files:** Create `cloud/src/litellm-portal/errors/error-boundary.tsx`, `cloud/src/litellm-portal/errors/error-boundary.test.tsx`; modify `cloud/src/litellm-portal/dashboard/views/usage-charts-lazy.tsx` (wrap the lazy chart island), `cloud/src/litellm-portal/dashboard/views/usage-dashboard.tsx` (wrap audit/dashboard core blocks — re-anchor by symbol, Task 1/3 already edited it).

- [ ] **Step 1: Write the failing ErrorBoundary test** — `cloud/src/litellm-portal/errors/error-boundary.test.tsx`:
...
- [ ] **Step 2: Run → FAIL** — `cd /Users/xumingyang/github/contrabass/cloud && bun run test src/litellm-portal/errors/error-boundary.test.tsx` → FAIL (module missing).

- [ ] **Step 3: Implement `cloud/src/litellm-portal/errors/error-boundary.tsx`** — a minimal class ErrorBoundary (React boundaries must be classes) whose fallback reuses the §F.3 humane-message style + a retry. NOT a god-boundary (one per critical block):
...
> The fallback wording follows the §F.3 3-element rule (what happened · the rest still works = input/context preserved · clear next step = 重试/刷新), no raw error. The `重试` resets the boundary (re-mounts children — a genuine recovery for transient runtime errors). New i18n keys (`该模块加载出错`, the description, `重试`) → both catalogs + `PHASE3_KEYS` (Task 9, cross-referenced).

- [ ] **Step 4: Wrap the critical blocks (additive; layered, NOT replacing MAJOR-1).** 
  - In `cloud/src/litellm-portal/dashboard/views/usage-charts-lazy.tsx`: wrap the `<React.Suspense …>` of each Lazy chart wrapper with `<BlockErrorBoundary blockLabel="用量图表">…</BlockErrorBoundary>` (boundary OUTSIDE Suspense so a render throw in the resolved chart is caught while the normal Suspense loading fallback still flows through untouched — the boundary is a transparent pass-through when there's no error, so the §E-1 loading-parity assertion in `hydration.test.tsx` is unaffected: SSR & client both still render the loading state, not the boundary fallback). Import `BlockErrorBoundary` from `../../errors/error-boundary`. The MAJOR-1 `client.tsx` `try/catch warmUsageCharts()` is **NOT touched** — comment in `usage-charts-lazy.tsx` MUST state the layering explicitly (chunk-fetch floor = client.tsx try/catch; render isolation = this boundary).
  - In `cloud/src/litellm-portal/dashboard/views/usage-dashboard.tsx`: wrap the dashboard core block (the resolved `!data ? loading : <charts+panels>` subtree — re-anchor by symbol; Task 1 added `ariaLabel`, Task 3 swapped to Lazy wrappers) in one `<BlockErrorBoundary blockLabel="用量看板">`. The audit core block lives in the Ops audit screen — wrap `ops-console/screens/audit.tsx`'s feed body in `<BlockErrorBoundary blockLabel="审计列表">` (file-disjoint from the dashboard wrap; re-anchor by symbol, Task 2 may have migrated its state branches to `Panel*`). Do NOT add a single page-level god-boundary.

- [ ] **Step 5: Run → PASS** — `cd /Users/xumingyang/github/contrabass/cloud && bun run test src/litellm-portal/errors/error-boundary.test.tsx src/litellm-portal/hydration.test.tsx src/litellm-portal/dashboard/charts/trend-chart.test.tsx` → PASS. **CRITICAL non-regression: `hydration.test.tsx` #418 loading-parity cases still green** (the boundary is a no-error pass-through on the loading path → SSR/hydrate first render unchanged; if these fail the boundary was placed wrong — it must NOT alter the loading-state render). `bun run typecheck` → baseline-only.

- [ ] **Step 6: `DESIGN.md` — note the layering.** Append to the `### Chart / Bundle 决策` section (after the §A.3 lazy/#418 record): `2026-05-17 §F.4: per-block React ErrorBoundary (errors/error-boundary.tsx) wraps the lazy chart island + dashboard/audit core blocks — RENDER/RUNTIME isolation, LAYERED ON TOP of the MAJOR-1 client.tsx try/catch (chunk-FETCH floor; unchanged). Orthogonal: fetch-fail → floor; render-throw → inline panel; siblings intact. Transparent on the loading/happy path so the §E-1 #418 loading-parity is unaffected.`

- [ ] **Step 7: Commit**
```bash
cd /Users/xumingyang/github/contrabass
git add cloud/src/litellm-portal/errors/error-boundary.tsx cloud/src/litellm-portal/errors/error-boundary.test.tsx cloud/src/litellm-portal/dashboard/views/usage-charts-lazy.tsx cloud/src/litellm-portal/dashboard/views/usage-dashboard.tsx cloud/src/litellm-portal/ops-console/screens/audit.tsx cloud/src/litellm-portal/DESIGN.md
git -c commit.gpgsign=false commit -m "feat(litellm-portal): per-block ErrorBoundary layered on chunk-fetch floor"
```
...
## Task 6B (§B band — §F.1 V2.0 §1.2): Ops B-端 cockpit density (compact Table + density toggle UI + keyboard)

> §B restyle band (after the §A GATE; serialized after Tasks 2/2C/6 — it consumes the Task-2 `densityClasses().cell/.row` tiers, the Task-2 `resolveDensity`+preference wiring (Step 9b), and the Task-6 restyled Ops shell). This is the §F.1 *visible* cockpit deepening; the *primitive* (table tiers + persisted resolver) already landed in Task 2.

**Files:** Create `cloud/src/litellm-portal/components/density-toggle.tsx`, `cloud/src/litellm-portal/components/density-toggle.test.tsx`; modify `cloud/src/litellm-portal/ops-console/shell.tsx` (mount the toggle in the Ops chrome), the Ops table screens `cloud/src/litellm-portal/ops-console/screens/{tenant-overview,audit,tenant-detail,user-detail}.tsx` (apply `densityClasses().cell/.row` + sticky header — file-disjoint per screen, parallel-wave), `cloud/src/litellm-portal/DESIGN.md`.

- [ ] **Step 1: Write the failing density-toggle test** — `cloud/src/litellm-portal/components/density-toggle.test.tsx`:
...
- [ ] **Step 2: Run → FAIL** — `cd /Users/xumingyang/github/contrabass/cloud && bun run test src/litellm-portal/components/density-toggle.test.tsx` → FAIL (module missing).

- [ ] **Step 3: Implement `cloud/src/litellm-portal/components/density-toggle.tsx`** — a small Kumo `Button`-based toggle (presentational + controlled; the persistence mutation is wired by the shell, keeping this unit-testable):
...
- [ ] **Step 4: Run → PASS** — `cd /Users/xumingyang/github/contrabass/cloud && bun run test src/litellm-portal/components/density-toggle.test.tsx` → PASS.

- [ ] **Step 5: Mount the toggle in the Ops chrome + wire persistence (Ops only).** In `cloud/src/litellm-portal/ops-console/shell.tsx` (re-anchor by symbol — Tasks 1/2/6 edited it): in the Ops page-head chip/summary row, render `<DensityToggle current={resolveDensity(densityPref, "compact")} onChange={(d) => updatePrefs.mutate({ density: d })} />` where `densityPref = usePreferences().data?.density` (already imported in Task-2 Step-9b) and `updatePrefs = useUpdatePreferences()` (from `../hooks/use-preferences`, the same hook `__root.tsx` uses for the theme toggle — verified). On mutate, the `PREFERENCES_QUERY_KEY` invalidates → `DensityProvider` re-resolves → density flips immediately, no full reload (the existing preferences mutation already does optimistic/refetch — mirror the theme toggle's pattern in `__root.tsx`). **Tenant Portal does NOT mount the toggle** (Tenant stays comfortable — §F.1; `tenant-portal/shell.tsx` is NOT modified by this task). SSR/#418: density rides the SSR-seeded `PREFERENCES_QUERY_KEY` (Task-2 Step-9b) so first render is consistent — covered by the Task-9 `hydration.test.tsx` re-run (theme toggle uses the identical channel and is already #418-proven; documented, not re-tested here).

- [ ] **Step 6: Apply compact table density to the Ops table screens (file-disjoint parallel-wave).** For each of `ops-console/screens/{tenant-overview,audit,tenant-detail,user-detail}.tsx` (re-anchor by symbol; Task 2 may have migrated their state branches to `Panel*`): read `const dc = densityClasses(useDensity());` and apply `dc.cell` to `Table.Cell`/`Table.Head` `className` and `dc.row` to `Table.Row`, and make the table header sticky (`<thead className="sticky top-0 bg-kumo-elevated z-10">` or Kumo `Table`'s sticky prop if present — read the Kumo `Table` API; if no sticky prop, the `sticky top-0` utility on the header row container is the fallback, Kumo-token bg only). `tabular-nums` already on numeric columns (Phase-2) — keep. These four screens are file-disjoint → parallel wave; per screen add/extend its colocated `*.test.tsx` asserting the compact cell class is applied when `density="compact"` (wrap render in `<DensityProvider density="compact">`), run that single test → PASS. Keyboard-flow: the existing row-link `Tab` order (Phase-2, covered by §A.1 Task 1) is preserved; do NOT add a roving-tabindex framework (§F.1 / V2.0 §4#4 — no over-abstraction). The command-palette (existing, `__root.tsx`) remains reachable — regression-only, no change.

- [ ] **Step 7: `DESIGN.md` — extend the Density Scale section with the cockpit model.** Append to `## Density Scale`: `§F.1 Ops cockpit: the compact tier applies `densityClasses().cell` (`px-3 py-1.5`) + `.row` (`h-9`, ≤36px) to all Ops Table screens with a sticky header; a comfortable↔compact `DensityToggle` (Ops chrome only — Tenant stays comfortable) persists via UserPreferences.density. "一屏看全/拒绝滚动" is achieved where data volume allows via compact rows + sticky header (table self-scrolls when long; outer shell does not).`

- [ ] **Step 8: Commit**
```bash
cd /Users/xumingyang/github/contrabass
git add cloud/src/litellm-portal/components/density-toggle.tsx cloud/src/litellm-portal/components/density-toggle.test.tsx cloud/src/litellm-portal/ops-console/shell.tsx cloud/src/litellm-portal/ops-console/screens cloud/src/litellm-portal/DESIGN.md
git -c commit.gpgsign=false commit -m "feat(litellm-portal): Ops B-端 cockpit density — compact tables + toggle"
```

---

## Task 6C (§B band — §F.6 V2.0 §1.3): ops-theme hardcoded hex → sanctioned named token + no-other-raw-hex guard

> Small task, §B band (file-disjoint: `ops-theme.ts` is touched by no other task; the guard test is new). Promotes the ONE approved raw-hex exception to named consts + a doc rationale, and adds a guard that no OTHER raw hex is introduced by the restyle.

**Files:** Modify `cloud/src/litellm-portal/ops-console/ops-theme.ts`; create `cloud/src/litellm-portal/ops-console/ops-theme.test.ts` (or extend an existing ops-theme test if present — check first); modify `cloud/src/litellm-portal/DESIGN.md`.

- [ ] **Step 1: Write the failing test** — `cloud/src/litellm-portal/ops-console/ops-theme.test.ts`:
...
- [ ] **Step 2: Run → FAIL** — `cd /Users/xumingyang/github/contrabass/cloud && bun run test src/litellm-portal/ops-console/ops-theme.test.ts` → FAIL (`OPS_STEEL_BRAND`/`OPS_STEEL_BRAND_HOVER` not exported; doc comment not yet the sanctioned wording). (The no-other-raw-hex case may pass or fail depending on current tree — if it fails, that surfaces pre-existing raw hex the restyle must also clean or allowlist with rationale; reconcile during execution.)

- [ ] **Step 3: Promote to named consts + sanctioned doc** — rewrite `cloud/src/litellm-portal/ops-console/ops-theme.ts` (value byte-unchanged):
...
(`OPS_STEEL_ACCENT`'s shape/value is byte-identical → every existing `style={OPS_STEEL_ACCENT}` consumer in `ops-console/shell.tsx` is UNCHANGED, no call-site edit, no type drift.)

- [ ] **Step 4: Run → PASS** — `cd /Users/xumingyang/github/contrabass/cloud && bun run test src/litellm-portal/ops-console/ops-theme.test.ts` → PASS (3 tests). If the no-other-raw-hex case lists offenders, they are either (a) pre-existing → fix to Kumo token or add to the allowlist WITH an in-test rationale comment, or (b) introduced by a §B restyle task → that task must use a Kumo token instead (no raw hex). Resolve before commit. `bun run typecheck` → baseline-only.

- [ ] **Step 5: `DESIGN.md` — record the sanctioned exception in the token table.** Add a row to the DESIGN.md color/token table (or a note under it): `| Ops fixed steel | OPS_STEEL_BRAND #475569 / OPS_STEEL_BRAND_HOVER #334155 | SANCTIONED raw-hex exception (§F.6 / V2.0 §1.3): Ops-only fixed brand voltage, Ops ignores tenant branding (Phase-2 invariant). The ONLY approved raw hex; all else via Kumo tokens (guarded). |`

- [ ] **Step 6: Commit**
```bash
cd /Users/xumingyang/github/contrabass
git add cloud/src/litellm-portal/ops-console/ops-theme.ts cloud/src/litellm-portal/ops-console/ops-theme.test.ts cloud/src/litellm-portal/DESIGN.md
git -c commit.gpgsign=false commit -m "refactor(litellm-portal): ops-theme hex → sanctioned named token + guard"
```
 ...
 * locale-driven chart aria template, the §F.1 density toggle, the §F.4
 * ErrorBoundary fallback, and EVERY §F.3 error-message contract string
 * (errors/error-messages.ts MAP values + GENERIC_FALLBACK_ID). Add a key
 * here AND to BOTH catalogs.
 */
export const PHASE3_KEYS = [
  ...
  // §F.1 density toggle (Task 6B)
  "切换显示密度", "紧凑", "宽松",
  // §F.4 BlockErrorBoundary fallback (Task 3C)
  "该模块加载出错", "重试",
  // §F.3 error-message contract (Task 2C) — EVERY value string in
  // errors/error-messages.ts MAP + GENERIC_FALLBACK_ID. The executor MUST
  // enumerate them from the FINAL error-messages.ts (the list below is the
  // verified-as-of-2026-05-17 set; reconcile 1:1 — every MAP value + the
  // fallback is a key here AND in both catalogs; the §F.3 test's
  // KNOWN_ERROR_CODES count floor + i18n-completeness pin the drift).
...
(During implementation, reconcile this list with the ACTUAL set of new `t\`…\`` / `<Trans>` strings introduced by Tasks 1–8 + the §F tasks (1/2C/3B/3C/6B/6C) — grep the touched files for new macro literals AND enumerate `errors/error-messages.ts`'s final `MAP` values + `GENERIC_FALLBACK_ID` 1:1; the list MUST be exactly the new Phase-3 + §F keys, no more, no less. Keys already in the Phase-1/2 catalogs — e.g. `"网络请求失败"` if it pre-exists — must NOT be double-added; verify presence before adding to the catalogs in Step 3. The §F.3 error-message strings are zh source ids consumed via `i18n._(id)`; en translations of each (preserving the 3-element structure) go in `en.ts` per Step 3.)
...
bun run test src/litellm-portal    # MUST be genuinely green. NO CI runs this suite
                                   # (verified: ci.yml=make test-quick (go+dashboard+landing);
                                   # cloud-ci.yml=build/deploy-dry; portal-e2e.yml=Playwright) —
                                   # THIS local sweep is the only gate; "CI covers it" is FALSE.
                                   # ONLY two by-name, isolated-re-run-confirmed exclusions:
                                   #  - happy-dom localhost:3000/kumo.css fetch-timeout flake (rotates
                                   #    across index/security/usage-overview SSR files Phase-3 did NOT
                                   #    modify) — re-confirm environmental via isolated re-run
                                   #  - pre-existing-on-main usage-overview ?window=90d (re-confirm it
                                   #    still fails identically on main; pre-dates Phase-3)
                                   # The pre-existing hydration.test.tsx "tenant_admin full-nav shell
                                   # at /" #418 is NOT an exclusion — Task 0B fixes it (7/7 required
                                   # from Task 0B on). Anything else red blocks §B/merge.
bun run test src/litellm-portal/tenant-portal/routes.test.tsx \
             ...
             src/litellm-portal/ops-console/ops-theme.test.ts \
...
bun run test:e2e:perf              # §F.2 CLS<0.1 HARD GATE on /,/usage,/ops (Playwright);
                                   # non-zero exit on CLS breach = CI red. LCP printed (observed).
```
Expected: all Phase-3 + §F suites genuinely green (incl. `errors/*` §F.3 + §F.4 ErrorBoundary, `a11y/type-scale.test.ts` §F.5, `components/density*.test.tsx` §F.1, `ops-theme.test.ts` §F.6, `hydration.test.tsx` **7/7** — Task 0B took the pre-existing "tenant_admin full-nav shell at /" #418 case RED→GREEN); the build prints `ECHARTS_LAZY_GATE: PASS`; `test:e2e:perf` exits 0 (CLS<0.1 all 3 routes); tsc baseline-only; build exit 0. `chart.tsx`/`app.test.tsx` are UNTOUCHED by Phase 3 (OQ1) — `app.test.tsx` must stay green for free. NO CI runs `cd cloud && bun run test` — this sweep is authoritative; the ONLY tolerated red is the two by-name isolated-confirmed exclusions (kumo.css fetch flake; pre-existing-on-`main` `?window=90d`), everything else (incl. all #418 cases) must be genuinely green.
...
- [ ] **Step 2: Manual visual + a11y staging checklist (for the user; after merge, deploy via `bun run deploy:litellm-portal` — NEVER bare `wrangler deploy`, which 500s):**
  ...
  8. **Invariants + #418 (NEW-C-A, CRITICAL — this is what the `fix/litellm-portal-hydration-418` branch exists for):** dark `data-mode` toggles cleanly on every new surface; zh/en both complete. **Hard-refresh (full SSR) `/usage` AND `/ops/usage` with devtools console open: ZERO React #418 / "hydration mismatch" / "did not match" warnings.** Per the verified Open-Question (b) the SSR renders the loading state and the client first-renders the same loading state (no #418 by construction; `warmUsageCharts` is defense-in-depth + UX, not the preventer). Also hard-refresh `/`, `/ops`, a tenant_admin shell, a member shell: no #418. (The automated `hydration.test.tsx` parity + negative-control cases — Task 3 Step 9 — gate this in CI; this manual check confirms the PRODUCTION `client.tsx` on a real browser, since the harness mirrors but is not the literal `client.tsx`.)
  9. **§F.5 type-scale 5-second & blur tests (V2.0 §1.1, human-judgment):** screenshot Tenant `/` and Ops `/ops`, light AND dark (4 shots). (a) **5s test:** show each for 5s then hide; reviewer names the primary CTA + main heading from memory — **pass = both correct for all 4**. (b) **Blur test:** apply ≈12px Gaussian blur to all 4; visual centre-of-mass must land on the primary content region (KPI/hero/table), NOT chrome/nav/decoration — **pass = content-region focal weight in all 4**. (Cross-ref: Task 1 Step 20.)
  10. **§F.3 error humanization (V2.0 §2.4 + §4#6):** force a few server errors (e.g. attempt an Owner tenant-write without impersonation → `impersonation_required`; request a missing billing period → `billing_archive_not_found`; with a DO offline → `index_do_unavailable`). The Banner/PanelError shows a **human 3-element message** (what happened + input-preserved cue when applicable + a clear recovery action) — **NEVER the raw code / DO name / HTTP number / stack**, zh AND en (`?lang=en`). An unknown/unexpected error shows the generic safe fallback, never a code.
  11. **§F.1 Ops cockpit density (V2.0 §1.2):** on `/ops` the tables render compact (dense rows ≤36px, tighter cells), header sticky on scroll; the comfortable↔compact toggle in the Ops chrome flips density immediately (no full reload) and **persists across reload** (preference saved). The Tenant Portal is UNAFFECTED (stays comfortable). Keyboard: `Tab` through an Ops table's row links follows visual order; the command-palette still opens.
  12. **§F.4 ErrorBoundary (V2.0 §2.4):** (dev/staging) force a runtime throw inside the chart block (e.g. temporarily feed malformed series, or use a devtools error-injection) → only that block shows the inline "该模块加载出错 + 重试" panel; the rest of the page stays interactive; clicking 重试 attempts recovery. Confirm this is the RENDER-isolation layer distinct from the chunk-fetch floor (block a chart-chunk network request in devtools → page still hydrates, no white screen — that is the MAJOR-1 try/catch floor; the two are layered, not duplicated).
  13. **§F.2 INP (V2.0 §2.1, observed — manual since not auto-measured):** on `/`, `/usage`, `/ops`, with devtools Performance/INP overlay, perform one representative interaction per route (window-selector change, nav click, density toggle). Record the INP value into the DESIGN.md observed-metrics record alongside the Task-3B CLS/LCP. INP < 100ms is the V2.0 target (observed, not a hard gate — a breach is a follow-up, not a merge blocker; CLS hard gate is the automated Task-3B one).
...
gh pr create --base main --head feat/litellm-portal-phase3-design-restyle \
  ...
  --body "Implements docs/superpowers/specs/2026-05-17-litellm-portal-phase3-design.md (APPROVED 2026-05-17, §E resolved; §F V2.0 6-item fold-in 2026-05-17). Hygiene-first: §A.1 a11y pass (focus ring, nav-mapped aria-current, ADDED aria to the real trend-chart.tsx, §F.5 type-scale audit ≤7 tiers/≤2×) → §A.2 unified Panel* state + density tokens (+ §F.1 compact-table tiers + persisted density pref) → §F.3 humanized server-error 3-element contract (shared extractError) → §A.3 lazy-split the real ECharts chain behind a CI HARD GATE IN build-litellm-portal-app.mjs (throws if echarts ∈ main / ∉ split; no KB threshold) + §F.2 Playwright CLS<0.1 HARD GATE (LCP/INP observed) + §F.4 per-block ErrorBoundary LAYERED on the MAJOR-1 chunk-fetch floor. Then §B restyle (shared SideNav, inline-SVG icons — Kumo has no icon set; brand summary bar / Ops steel pill; §F.1 Ops B-端 cockpit density + comfortable↔compact toggle; §F.6 ops-theme hex → sanctioned named token + no-other-raw-hex guard) + §C extended 6-surface brand guard. Also fixes a PRE-EXISTING shipped #418 (Kumo SkeletonLine unseeded-Math.random on the SSR shell path — Task 0B, since no CI runs this vitest suite and #418 is the branch invariant) and adds a Tenant pathless layout symmetric to Ops OpsLayout (Task 0, structural prerequisite). No Kumo fork, no semantic/type-token override; Ops steel/ignore-branding untouched; #418 §A.3 workstream unchanged except §F.4's additive ErrorBoundary. SSR/#418 + bilingual + dark-mode + WCAG-AA invariants held. Depends on #139+#140 (both in main). Tests: all Phase-3+§F suites genuinely green incl. hydration.test.tsx 7/7; build prints ECHARTS_LAZY_GATE PASS; CLS<0.1 e2e gate green; tsc baseline; SPA regenerated/committed. §F.3 error contract security-reviewed (retained pass). No CI runs the cloud vitest suite — the §A-GATE local sweep is authoritative; the only tolerated red is two by-name isolated-confirmed exclusions (kumo.css fetch flake; pre-existing-on-main ?window=90d), not 'CI covers it'."
```
...
Order is fixed: **typecheck → build (the build IS the §A.3 ECharts gate) → full test → §F.2 `test:e2e:perf` (CLS<0.1 HARD GATE) → isolate flakes → manual staging checklist (incl. §F.5 5s/blur, §F.3 error humanization, §F.1 cockpit, §F.4 ErrorBoundary, §F.2 INP observed) → deploy**. There are now TWO automated §A-band hard gates: (1) the §A.3 ECharts-lazy assertion living **inside `cloud/scripts/build-litellm-portal-app.mjs`** (the real `client.tsx` `splitting:true` build): it `throw`s (non-zero, fails `bun run build:litellm-portal` and CI) if echarts is in the `main` chunk or absent from every split chunk, printing `ECHARTS_LAZY_GATE: PASS/FAIL`; NO KB threshold — the gzip delta is recorded in `DESIGN.md` as observational only. `analyze-litellm-portal-bundle.mjs` is observational-only (`app.tsx` single-bundle, NOT the gate — C2). (2) the §F.2 CLS<0.1 Playwright assertion (`bun run test:e2e:perf`, Task 3B) — non-zero exit on a CLS breach on `/`,`/usage`,`/ops`; LCP/INP are observed-and-recorded (not gates), consistent with §A.3's observed-metric precedent. Both gate §B (the §A GATE includes them). **GATE REALITY: no CI workflow runs `cd cloud && bun run test`** (`ci.yml`=`make test-quick`=go+dashboard+landing; `cloud-ci.yml`=build/deploy-dry; `portal-e2e.yml`=Playwright) — the §A-GATE local vitest sweep is the ONLY gate; "CI is green so it's fine" is FALSE and is never the justification. The pre-existing shipped `hydration.test.tsx` "tenant_admin full-nav shell at /" #418 is therefore NOT tolerable as "environmental" — it is FIXED in Task 0B (7/7 required from Task 0B on). The ONLY two tolerated exclusions are by-name, isolated-re-run-confirmed: the happy-dom `localhost:3000/kumo.css` fetch-timeout flake (Phase-3 does not modify those files) and the pre-existing-on-`main` `usage-overview ?window=90d` failure (re-confirm it still fails identically on `main`); everything else must be genuinely green. The manual visual/a11y staging checklist (Task 10 Step 2) is for the user — it asserts structural/behavioral/a11y/contrast outcomes (focus ring, `aria-current`, density, lazy network behavior, all-or-nothing brand fallback, #418), never pixels. Deploy is `bun run deploy:litellm-portal` only (bare `wrangler deploy` 500s — project memory). PR-merge follows the standing Phase-2/Phase-3 authorization once gates are green; do not self-merge beyond that.
...
| **§F.1** V2.0 §1.2 | B-端 cockpit density: `densityClasses().cell/.row` compact-table tiers + `resolveDensity` SSR-safe resolver + `UserPreferences.density` persisted pref (Task 2 Steps 5/7/9/9b); `DensityToggle` Ops-chrome control + compact tables/sticky header on 4 Ops table screens + keyboard-flow preserved (Task 6B). Tenant stays comfortable | Task 2 + Task 6B | ✓ (primitive in §A.2, visible cockpit in §B) |
| **§F.2** V2.0 §2.1 | CLS<0.1 = automated HARD GATE (Playwright `e2e/perf-budget.spec.ts`, `bun run test:e2e:perf`, in the §A GATE); LCP observed-and-recorded; INP observed via Task-10 manual item. No Lighthouse/new dep | Task 3B | ✓ (CLS hard; LCP/INP honestly observed-only) |
| **§F.3** V2.0 §2.4 + §4#6 | error-code → human 3-element message single-source `errors/error-messages.ts` + shared `errors/extract-error.ts` (replaces the 2 duplicated locals) + `PanelError` i18n-resolves; no-tech-leak + unknown-code-fallback + permission-recovery tests; zh/en. **SECURITY/CONTRACT-SENSITIVE → retained review pass (marked on the task)** | Task 2C | ✓ (cures §2.4 + §4#6 abstraction-leak) |
| **§F.4** V2.0 §2.4 | `BlockErrorBoundary` per critical block (lazy chart / dashboard / audit), inline humane fallback + retry, siblings intact. LAYERED ON the MAJOR-1 chunk-fetch try/catch (unchanged); transparent on the loading path so §E-1 #418 parity is unaffected (asserted by re-running `hydration.test.tsx` in Task 3C Step 5) | Task 3C | ✓ (additive; #418 workstream untouched) |
| **§F.5** V2.0 §1.1 | type-scale audit: `a11y/type-scale.test.ts` reads the real DESIGN.md Typography table, asserts ≤7 tiers + heading:body ≤2× (current = 8 tiers / 2.14–2.25× — converged by merging the two 12px label rows + capping Page title to 28→32px = 2.0×, documented; no Kumo fork/token-override); 5s + blur tests as Task-10 manual items with concrete pass bars | Task 1 Steps 18–20 | ✓ (real-source assertion; honest convergence, not assumed) |
| **§F.6** V2.0 §1.3 | `ops-theme.ts` hex → `OPS_STEEL_BRAND`/`_HOVER` named consts + sanctioned-exception doc + DESIGN.md token-table row; `ops-theme.test.ts` asserts the consts + a no-other-raw-hex grep guard (allowlist = ops-theme + branding.ts WCAG constants). Value byte-unchanged (Ops fixed-steel/ignores-branding invariant preserved) | Task 6C | ✓ (named exception, not value change) |

| **Task 0** structural | Tenant pathless `tenantLayoutRoute` mirroring `OpsLayout`/`opsLayoutRoute` so the shell/SideNav/`<main>` wrap `/usage|/keys|…`; `/` stays `indexRoute`; no route-id collision; `PortalIndex` byte-unchanged; `tenant-portal/routes.test.tsx` topology guard | Task 0 | ✓ (idiom mirrored 1:1) |
| **Task 0B** pre-existing #418 | Kumo `SkeletonLine` unseeded-`Math.random` SSR mismatch (shipped, RED at `d3e6b55`/`b46b998`/HEAD) fixed by `useHasHydrated` + `SsrSafeSkeleton` (deterministic SSR placeholder → post-hydration swap; NO `suppressHydrationWarning`); `overview.tsx` SSR-path branches migrated; Task-2 `PanelSkeleton` built on `SsrSafeSkeleton` so §A.2 convergence inherits the safety; oracle = unchanged `hydration.test.tsx` 7/7. RETAINED-REVIEW (locked #418 invariant) | Task 0B (+ Task 2 coupling) | ✓ (root-cause fix, not band-aid; no CI backstop so genuinely gated) |

No §A–§E (or §F / Task-0 / Task-0B) gap. No new screens / no backend / no IA change / no Kumo fork / no semantic-hierarchy-typography token override (asserted by Task 1's no-`--kumo-*`-except-brand invariant + Task 8's only-`--kumo-brand*`-emitted guard). §B not started before §A passes (the explicit §A GATE block after Task 3). §C depends on §B.1/§B.2 surfaces (Task 8 Step 5 verifies they consume `--kumo-brand`).

**2. Placeholder scan (honest, post-critic):** No "TBD"/"待定"/"add appropriate X"/"similar to Task N"/uncoded steps. The icon source is NO LONGER an "impl-time decision / probe" (the prior false defense) — C3 was verified: Kumo has no icon set, so Task 4 ships a COMPLETE 10-slot zero-dependency inline-SVG `nav-icons.tsx` verbatim (no probe, no placeholder, no deferred choice). Remaining disclosed executor-resolved items, each with a deterministic in-step procedure (NOT a hand-wave): (a) Task 8 Step 3 — the four elevated/tint surface luminances: read the real `@cloudflare/kumo@2.1.0` theme-kumo.css oklch values for `--color-kumo-elevated`/`--color-kumo-tint` (light & dark) and convert them the documented way the existing canvas constants were derived; the hex literals shown are explicitly flagged as to-be-replaced-with-real-values WITH the derivation method; (b) §C test witness colors are flagged as swappable witnesses for an invariant ("compute `contrastRatio` against the Step-3 luminances; pick values that unambiguously pass/fail; do not weaken the assertion"); (c) the `usage-charts-lazy.tsx` per-chart prop types — read `rank-bar.tsx`/`model-donut.tsx` and copy their exact prop types (no `any`/`@ts-ignore`, repo hard rule); (d) Task 1 Step 14's chart-aria inputs — RESOLVED concretely (post-critic): `DashboardResponseSchema` has NO `windowLabel` (verified `dashboard-schemas.ts:17-42`); the window label source is the `win` state value itself (`WindowSelector`/`panel.tsx` renders the raw `DashboardWindow` as its label — no separate map), `grain: data?.grain` is valid (`z.enum(["hour","day"])`). The Step-14 sample + anchor note now name these concrete sources (not "figure it out"). No prose-only step; no undefined symbol; no deferred design choice.
...
**9. V2.0 §F fold-in disposition (2026-05-17 amendment — HONEST, no asserted-✓-without-encoding):** 6 items folded into BOTH the spec (new §F + §C.3 note + status + Self-Review) and the plan (Task 1 Steps 18–21 add §F.5; Task 2 Steps 5/7/9/9b add §F.1 primitive; new Tasks **2C/3B/3C/6B/6C** add §F.3/§F.2/§F.4/§F.1-cockpit/§F.6). **Task count: 10 → 15** (the 5 lettered tasks; item-5 folded into Task 1, item-1's density primitive into Task 2 — NOT separate tasks, honestly counted as folds not new tasks). Encoded-vs-claimed honesty:
  - **§F.5 (type-scale):** the test READS the real DESIGN.md table and the convergence is a concrete edit (merge the two 12px label rows → 7 tiers; Page title 30→36 ⇒ 28→32 = exactly 2.0× / cap). I did NOT claim "already ≤7/≤2×" — the spec & plan explicitly state current = 8 tiers / 2.14–2.25× and give the deterministic convergence + a documented-exception fallback if a merge can't land. Real `DESIGN.md` numbers were verified by reading the file.
  - **§F.2 (perf):** CLS is the ONLY hard gate (deterministic, Playwright-assertable); LCP observed; INP honestly downgraded to a manual observed item (cannot be reliably auto-measured without a scripted interaction — stated, not hand-waved). Reuses the verified-present `@playwright/test`+`playwright.config.ts`; no new dep. The Step-1 "read playwright.config.ts first / specify minimal webServer if absent" is a real instruction, not a placeholder.
  - **§F.3 (error contract):** complete real code (the `MAP`, `errorMessage`, shared `extractError`), signature byte-identical to the 2 removed locals (no call-site/type drift — verified both have `extractError(json, fallback): string`), explicit catalog/i18n wiring, marked **SECURITY/CONTRACT-SENSITIVE for the retained review pass** (the task header + spec §F.3 + the §F.3 coverage row all carry the marker). The ~27-code enumeration was taken from a real `rg` of `routes.ts`; the test pins drift via a count floor.
  - **§F.4 (ErrorBoundary):** ADDITIVE, explicitly LAYERED on the closed MAJOR-1 try/catch (which is NOT modified — Task 3 / `client.tsx` Step 5 untouched by Task 3C). The boundary is a no-error pass-through on the loading path so the §E-1 #418 loading-parity is provably unaffected (Task 3C Step 5 re-runs `hydration.test.tsx` as the CRITICAL non-regression). The #418 §A.3 workstream is otherwise untouched (no edit to the warm-up, the `not.toContain` pins, OQ-(b) reasoning, the bundle gate).
  - **§F.1 (cockpit):** primitive (table tiers + `resolveDensity` + persisted pref via the EXISTING preferences channel — verified `usePreferences`/`PREFERENCES_QUERY_KEY` SSR-seed) in §A.2 Task 2; visible toggle+compact-tables in §B Task 6B (post-gate). SSR/#418: density rides the same seeded `PREFERENCES_QUERY_KEY` as the already-#418-proven theme toggle — documented as structurally-covered (not a new heavy harness, honestly scoped).
...
**10. Structural defect fix — Tenant pathless layout (2026-05-17, execution-time discovery; decision LOCKED by lead):**
  ...
  - **Task 1 deltas:** (i) Task-0 dependency stated; (ii) Steps 1–5 left EXACTLY as-is (lead's in-flight implementer completed them green — compatible, not re-touched); (iii) the new `renderTenantAt`/`renderOpsAt` harnesses got the MISSING `await router.load()` (the Phase-2-proven step from `ops-console/routes.test.tsx:58-74` the original draft omitted) — helpers made `async`, all call sites `await`ed; (iv) NEW **Step 9c** migrates the **8 pre-existing bare-shell `it`s across 3 files** (incl. the originally-missed `tenant-portal/shell-impersonation.test.tsx` ×3, migrated via `renderTenantAt("/usage")` + seeded `me.impersonation` set/null/omitted — RESOLVED OQ: `MeSchema.impersonation` is `.nullable().optional()`, fully seedable — preserving the file's `Toasty` + `useStopImpersonation` `vi.mock`s) to the production-router harness (drive by path+seeded `me`, assertions UNCHANGED — same nav counts/403/Phase-2-notice/banner-present-or-absent; only the render mechanism bare→router), with a mandatory `rg -l 'TenantPortalShell|OpsConsoleShell' src/litellm-portal --glob '*test*'` live-census reconciliation (do not trust the hardcoded count); Step 10 rewritten to require all 8 migrated + the new a11y `it`s green (no `isServer`); `shell-impersonation.test.tsx` added to BOTH the §A GATE sweep and the Task-10 sweep so the migration is GATED; (v) §F.5 Steps 18–21 untouched.
  ...
  - **Untouched (verified):** the ✅ #418/§A.3 workstream (warm-up, `not.toContain` §A-gate pins, OQ-(b) reasoning, `hydration.test.tsx` parity/negative-control, the build gate) and the just-folded §F items are NOT modified by this fix. Task 0's pathless layout is identity-derived/SSR-safe IDENTICALLY to `OpsLayout` (no new #418 surface — `hydration.test.tsx` at Task-0 Step-5 is still 6/7 baseline, the 1 red is the UNCHANGED pre-existing case, fixed by Task 0B). `PortalIndex` (the `/`-owner) is byte-unchanged. Self-reviewed against the live route files (`tenant-portal/routes.tsx`, `ops-console/routes.tsx`, `router.tsx`, `routes/index.tsx`, `ops-console/routes.test.tsx`) — anchors real, OpsLayout idiom mirrored 1:1, no decision re-opened.

**11. Pre-existing shipped #418 remediation — Task 0B (2026-05-17, execution-time discovery; authorized into Phase 3 by the user's decision rule):**
  - **Defect (verified empirically by the lead at THREE points — current tree, clean `b46b998`, `d3e6b55` = Phase-3 branch start = shipped Phase-1/2 `main`):** `hydration.test.tsx` "tenant_admin full-nav shell at /" (1 of 7 `#418 guard` cases) is RED in SHIPPED production — NOT introduced by Phase 3 / Task 0 / Task 1. Root cause: `PortalIndex`→`<TenantPortalShell><TenantOverviewScreen/></TenantPortalShell>`; `overview.tsx` tile loading branches render Kumo `SkeletonLine`; Kumo `SkeletonLine` (`skeleton-line-*.js:5`) computes `--skeleton-width`/`--shimmer-*` from UNSEEDED `Math.random()` → SSR ≠ client first render → genuine #418. `test/setup.ts` does not seed `Math.random`; no SSR-stable Kumo mode; no `useHasHydrated` existed.
  - **Why fixed in Phase-3 (not deferred):** VERIFIED no CI workflow runs `cd cloud && bun run test` (`ci.yml`=`make test-quick`; `cloud-ci.yml`=build/deploy-dry; `portal-e2e.yml`=Playwright). The §A-GATE local sweep is the ONLY gate; #418 is the branch's namesake invariant. A carve-out would mean shipping the branch's own bug behind a green-looking gate — unacceptable. User decision rule authorized fixing it inside Phase 3.
  - **Fix shape (root-cause, NOT band-aid):** new `useHasHydrated()` (false on SSR+client-first-render, true post-`useEffect`) + `SsrSafeSkeleton` (deterministic `Math.random`-free `<div>` placeholder on SSR+first-render, swaps to the real Kumo `SkeletonLine` post-hydration → randomness never participates in hydration). `suppressHydrationWarning` explicitly FORBIDDEN. Applied to ONLY the Step-1-pinpointed SSR-path `overview.tsx` branches (minimal, no broad refactor). Same discipline family as Task-3's "dynamic work after hydrate". TDD; PRIMARY oracle = the UNCHANGED `hydration.test.tsx` going 6/7 → 7/7 (the other 6 stay green; if any regress the fix touched too much).
  - **Task-2 coupling (critical, encoded):** Task 2's `PanelSkeleton` (`components/panel-state.tsx`) is built ON `SsrSafeSkeleton`, NOT raw Kumo `SkeletonLine` — so the §A.2 convergence (which migrates `overview.tsx`'s skeletons INTO `PanelSkeleton`) INHERITS the #418-safety instead of re-introducing the bug. Encoded in Task-2 Step-3 code + the DESIGN.md State-Treatments row + the File-Structure rows.
  - **Cross-cutting updates:** task list now **17** (Task 0, **Task 0B**, 1–10, 2C/3B/3C/6B/6C); §A chain Task 0 → Task 0B → Task 1; Decomposition note + §A-GATE narrative + Task-10 sweep updated; the FALSE "CI is the gate / environmental, so it's fine" rationale REMOVED everywhere (Prerequisites GATE-REALITY line, Task-0 Step-5, §A-GATE block, Task-10 Step-1 comment+Expected, PR body, E2E narrative) and replaced with the accurate "the §A-GATE local sweep is authoritative; only TWO by-name isolated-confirmed exclusions (kumo.css fetch flake; pre-existing-on-`main` `?window=90d`); the pre-existing #418 is NOT an exclusion — Task 0B fixes it". The two genuinely-environmental flakes remain documented but require isolated re-confirmation, not a CI hand-wave. RETAINED-REVIEW marked (locked #418 invariant, Task-2C tier).
  - **Untouched (verified):** Task 3 / §E-1 / MAJOR-1 try/catch / OQ-(b) / Task 0 / the §F task bodies (2C/3B/3C/6B/6C) are NOT modified by Task 0B. `hydration.test.tsx` is NOT edited (it is the unchanged oracle; Task 3 separately EXTENDS it for NEW-C-A — Task 0B adds nothing to it). Self-reviewed against the live files: `hydration.test.tsx` (the failing case + harness), `routes/index.tsx` `PortalIndex`, `tenant-portal/screens/overview.tsx` (`SkeletonLine`@~64-68/113-117/167-174), `node_modules/@cloudflare/kumo/.../skeleton-line-*.js:5` (the `Math.random`), `src/test/setup.ts` (no seed), and a repo-wide grep confirming no pre-existing `useHasHydrated`/mounted hook — the spec'd files/anchors/mechanism are real.
