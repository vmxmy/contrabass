...
- [ ] 1.2 Generate Go types from those schemas under `internal/workerproto/v1/` and add `go generate` step
- [ ] 1.3 Generate TypeScript types from the same schemas under `cloud/src/workerproto/v1/` and wire into the cloud package's `tsconfig`
- [ ] 1.4 Add a contract-test fixture set (golden JSON for every frame) under `testdata/workerproto/v1/` consumed by both Go and TS test suites
...
- [ ] 3.1 Create D1 database `contrabass-control-plane` and add binding to `wrangler.toml`
...
- [ ] 4.1 Create R2 buckets: `contrabass-artifacts` (logs/diffs/screenshots), `contrabass-events-archive`
...
- [ ] 5.1 Create Secrets Store and document per-team secret naming convention (`tracker/{teamId}/linear`, `tracker/{teamId}/github`)
...
- [ ] 6.1 Implement `IssueRun` DO class under `cloud/src/do/issue-run.ts` with state machine `queued | dispatched | running | succeeded | failed | cancelled`
...
- [ ] 7.1 Implement `TeamCoordinator` DO class under `cloud/src/do/team-coordinator.ts`
- [ ] 7.2 Implement worker registry (in-memory + DO storage) with `idle | busy | unhealthy` status
