...
type workerAckingDispatchHandler struct {
	...
	mu       sync.Mutex
	inFlight map[workerv1.RunID]struct{}
}
...
func newWorkerAckingDispatchHandler(
	...
	return &workerAckingDispatchHandler{
		...
		startRun:       startRun,
		inFlight:       make(map[workerv1.RunID]struct{}),
	}
}
...
func (h *workerAckingDispatchHandler) Handle(ctx context.Context, frame workerv1.WorkerDispatchFrame) error {
	...
	go func() {
		...
		_ = h.startRun(ctx, frame)
	}()
	return nil
}

func (h *workerAckingDispatchHandler) tryAcquire(runID workerv1.RunID) bool {
	h.mu.Lock()
	defer h.mu.Unlock()
